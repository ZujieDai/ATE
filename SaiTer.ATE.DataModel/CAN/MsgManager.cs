using SaiTer.ATE.DataModel.CAN.MsgSorts;
...
namespace SaiTer.ATE.DataModel.CAN
{
    public class MsgManager
    {
        ...
        public CanMsgRich DecodeMsgData(string flowId, List<byte> content, ESGBDC_Ver BMSGB_Ver)
        {
            ...
            try
            {
                ...
                {
                    model = machine.DecodeMsgData(symbol, content,BMSGB_Ver);
                ...
                else //多包 正文
                {
                    ...
                    if (index == CommonCAN.MutiPackage.GetCountPlan())    //最后一包
                    {
                        CommonCAN.MutiPackage.AppendContentPackage(content);  //先附加包，再解析
                        machine = CreateDecodeMsgMachine(symbol);
                        model = machine.DecodeMsgData(symbol, CommonCAN.MutiPackage.GetMutiContent(), BMSGB_Ver);    //解析所有包
                        model.ConsistMsg.MutiLength = CommonCAN.MutiPackage.GetAppendCnt();               //获取实际包数 for Consist Test
                    }
                    else
                    {
                        model = machine.DecodeMsgData(symbol, content, BMSGB_Ver);     //先解析当前包：for解析出第几包
                        CommonCAN.MutiPackage.AppendContentPackage(content);  //再附加包
        ...
        private string FindMsgSymbol(string flowId, List<byte> content)
        {
            try
            {
                ...
                {
                    CommonCAN.MutiPackage.UpdateMutiPackage_Head(content);    //更新多包信息
                    symbol = CommonCAN.MutiPackage.GetSymbol();  //多包symbol
                }
                else if (flowId == CanMsgId.MUTI_PACKAGE_READY)
                {
                    CommonCAN.MutiPackage.UpdateMutiPackage_Ready(content);
                    symbol = CommonCAN.MutiPackage.GetSymbol();
                }
        ...
        private int SetPackageId(string flowId)
        {
            ...
            {
                return CommonCAN.MutiPackage.GetCurrentPackageId();
            }
            else
                return 0;
        }
        private int SetPackageTextId(string flowId)
        {
            if (flowId == CanMsgId.MUTI_PACKAGE_TEXT)
            {
                return CommonCAN.MutiPackage.GetCurrentTextId();
            }
            else
                return 0;
        }
