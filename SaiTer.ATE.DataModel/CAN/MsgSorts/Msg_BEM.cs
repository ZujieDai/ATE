    ...
    public class Msg_BEM : MsgCommon
    {
        ...
        public override CanMsgRich DecodeMsgData(string symbol, List<byte> content, ESGBDC_Ver eSGBDC_Ver)
        {
            ...
            try
            {
                ...
                string state = Function.MatchState(result, SPN.StateName.SPN3901);
                text += Function.TextAddColonSpace(TestRecognize00, state);

                result = BaseConvert.GetBitsFromHex(val, 2, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3902);
                ...
                state = Function.MatchState(result, SPN.StateName.SPN3903);
                text += Function.TextAddColonSpace(TestSync, state);

                result = BaseConvert.GetBitsFromHex(val, 2, 2);
                model.ConsistMsg.SPN3904 = result;
                state = Function.MatchState(result, SPN.StateName.SPN3904);
                ...
                state = Function.MatchState(result, SPN.StateName.SPN3905);
                text += Function.TextAddColonSpace(TestState, state);

                result = BaseConvert.GetBitsFromHex(val, 2, 2);
                model.ConsistMsg.SPN3906 = result;
                state = Function.MatchState(result, SPN.StateName.SPN3906);
                ...
                state = Function.MatchState(result, SPN.StateName.SPN3907);
