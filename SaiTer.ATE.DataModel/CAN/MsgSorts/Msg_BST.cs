...
namespace SaiTer.ATE.DataModel.CAN.MsgSorts
{
    public class Msg_BST : MsgCommon
    {
        ...
        private string TestParamErr = "充电参数不匹配";
        ...
        public override CanMsgRich DecodeMsgData(string symbol, List<byte> content, ESGBDC_Ver eSGBDC_Ver)
        {
            ...
            try
            {
                ...
                state = Function.MatchState(result, SPN.StateName.SPN3512_9);
                text += Function.TextAddColonSpace(TestBatTemp, state);

                result = BaseConvert.GetBitsFromHex(val, 2, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3512_11);
                text += Function.TextAddColonSpace(TestRelay, state);

                result = BaseConvert.GetBitsFromHex(val, 4, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3512_13);
                text += Function.TextAddColonSpace(TestDetectPoint2, state);

                result = BaseConvert.GetBitsFromHex(val, 6, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3512_15);
                ...
                result = BaseConvert.GetBitsFromHex(val, 0, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3512_12);
                text += Function.TextAddColonSpace(TestInsulateProblem, state);

                result = BaseConvert.GetBitsFromHex(val, 2, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3512_34);
                text += Function.TextAddColonSpace(TestConnOverTemp, state);

                result = BaseConvert.GetBitsFromHex(val, 4, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3512_56);
                text += Function.TextAddColonSpace(TestBmsOutputConn, state);

                result = BaseConvert.GetBitsFromHex(val, 6, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3512_78);
                ...
                result = BaseConvert.GetBitsFromHex(val, 0, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3513_12);
                text += Function.TextAddColonSpace(TestCurrent, state);

                result = BaseConvert.GetBitsFromHex(val, 2, 2);
                state = Function.MatchState(result, SPN.StateName.SPN3513_34);
                ...
                switch (eSGBDC_Ver)
                {
                    ...
                    case ESGBDC_Ver.GBDC_2023:
                        result = BaseConvert.GetBitsFromHex(val, 4, 2);
                        state = Function.MatchState(result, SPN.StateName.SPN3513_56);
                        text += Function.TextAddColonSpace(TestParamErr, state);
                        break;
