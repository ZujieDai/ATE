    ...
    public class Msg_BRM : MsgCommon
    {
        ...
        public override CanMsgRich DecodeMsgData(string symbol, List<byte> content, ESGBDC_Ver eSGBDC_Ver)
        {
            ...
            try
            {
                ...
                //10.车辆识别码(VIN)2015 ,扩展车辆识别码(EVIN)2023
                switch (eSGBDC_Ver)
                {
                    ...
                    case ESGBDC_Ver.GBDC_2023:
                        TestVin= "扩展车辆识别码(EVIN)";
                        break;
                }
                string vin = DecodeVin(arr);
                text2 += Function.TextAddColonSpace(TestVin, vin);
                string testtext = text + text2;
                model.MsgText = Function.AppendTextToMsgHead(symbol, MsgHeadLine) + LastPckgText + Punctuation.Space + testtext;
                return model;
            }
            catch (Exception ex)
            {
