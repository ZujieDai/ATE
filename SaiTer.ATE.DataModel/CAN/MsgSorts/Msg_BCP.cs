    ...
    public class Msg_BCP : MsgCommon
    {
        ...
        public override CanMsgRich DecodeMsgData(string symbol, List<byte> content, ESGBDC_Ver eSGBDC_Ver)
        {
            ...
            try
            {
                ...
                model.MsgText = Function.AppendTextToMsgHead(symbol, MsgHeadLine) + LastPckgText + Punctuation.Space + text;
                return model;
            }
            catch (Exception ex)
            {
        ...
        {
            int current = BaseConvert.HexStr2Int32(sHigh + sLow) - (int)MsgConfig.mCurrentRefrence * 10;//偏移量
            double shrinkC = Function.ShrinkCntTimes(current, 10);
            double result = Function.KeepCntDecimalPlaces(shrinkC, 1);
