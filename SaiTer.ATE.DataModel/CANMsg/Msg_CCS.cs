    ...
    public class Msg_CCS : MsgCommon
    {
        ...
        public override CanMsgRich DecodeMsgData(string symbol, List<byte> content)
        {
            ...
            try
            {
                ...
                text += PacketHelper.TextAddColonSpace(TestOutputVolt, outputV);

                string outputI= DecodeOutputCurrent(arr[i++], arr[i++]);
                text += PacketHelper.TextAddColonSpace(TestOutputCurrent, outputI);

                string chargeT = DecodeChargeTime(arr[i++], arr[i++]);
                text += PacketHelper.TextAddColonSpace(TestChargeTimeSum, chargeT);
                ...
                text += PacketHelper.TextAddColonSpace(TestChargePermit, chargePermit);
