...
namespace SaiTer.ATE.Business
{
    ...
    /// 交流老化测试流程
    /// </summary>
    public class AgingTest_AC : BusinessBase
    {
        public AgingTest_AC(int trialType) { TrialType = trialType; }
        ...
        public override void InitializeParams()
        {
            Init();
            //
            //老化电压(V)=220|老化电流(A)=10|老化时间(分)=3|监测数据间隔时间(秒)=30|电压误差(%)=5|电流误差(%)=5
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                SetLoadPara(testWorkParam.lstIDs, AgingVolt - 20, AgingCurr, AgingVolt, AgingCurr);
                Thread.Sleep(1000);
                ...
                {
                    double voltage = AllEquipStateData.DicBMS_AC_StateData[itmp].PhaseA_Voltage;
                    if (voltage == 0)
                    {
                        Thread.Sleep(500);
                        voltage = AllEquipStateData.DicBMS_AC_StateData[itmp].PhaseA_Voltage;
                    }
                    dicAgingVolt.Add(itmp, voltage.ToString("F2"));

                    double current = AllEquipStateData.DicBMS_AC_StateData[itmp].PhaseA_Current;
                    if (current == 0)
                    {
                        Thread.Sleep(500);
                        current = AllEquipStateData.DicBMS_AC_StateData[itmp].PhaseA_Current;
                    }
