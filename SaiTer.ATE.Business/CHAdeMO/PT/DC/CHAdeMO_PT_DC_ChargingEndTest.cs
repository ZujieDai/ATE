...
namespace SaiTer.ATE.Business
{
    ...
    /// 充电结束测试
    /// </summary>
    public class CHAdeMO_PT_DC_ChargingEndTest : BusinessBase
    {
        public CHAdeMO_PT_DC_ChargingEndTest(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    string stmp = "";
                    stmp = AllEquipStateData.DicBMS_JP_DC_StateData[testWorkParam.lstIDs.First()].ChargingVoltage <= 20 ? "是" : "否";
                    ProcessDataResult(testWorkParam.lstIDs, stmp, "充电桩电压是否小于20V", stmp == "是" ? true : false, "充电结束后");

                    stmp = AllEquipStateData.DicBMS_JP_DC_StateData[testWorkParam.lstIDs.First()].ChargingVoltage <= 10 ? "是" : "否";
                    ProcessDataResult(testWorkParam.lstIDs, stmp, "电子锁解锁前电压是否小于10V", stmp == "是" ? true : false, "充电结束后");
