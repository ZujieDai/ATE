...
namespace SaiTer.ATE.Business
{
    ...
    /// 正常充电时序
    /// </summary>
    public class CHAdeMO_PT_DC_ChargingTiming : BusinessBase
    {
        public CHAdeMO_PT_DC_ChargingTiming(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    ProcessDataResult(testWorkParam.lstIDs, stmp, "是否正常启动充电", stmp == "是" ? true : false, "正常充电时序测试");
                    ...
                    ProcessDataTmp(d1, "充电阶段", "输出电压(V)", (BMSDemandVoltage * 0.9).ToString(), (BMSDemandVoltage * 1.1).ToString());
                    ...
                    ProcessDataTmp(d1, "充电阶段", "输出电流(A)", (BMSDemandCurrent * 0.9).ToString(), (BMSDemandCurrent * 1.1).ToString());
                    ...
                    stmp = ChangeBMSChargeStatus_JP_DC(AllEquipStateData.DicBMS_JP_DC_StateData[testWorkParam.lstIDs.First()].SystemState) != 7 ? "是" : "否";
                    ProcessDataResult(testWorkParam.lstIDs, stmp, "是否正常结束充电", stmp == "是" ? true : false, "充电结束后");
                    ...
                    ProcessDataTmp(d1, "充电结束后", "输出电压(V)", "-", "20");
                    ...
                    ProcessDataTmp(d1, "充电结束后", "输出电流(A)", "-", "2");
