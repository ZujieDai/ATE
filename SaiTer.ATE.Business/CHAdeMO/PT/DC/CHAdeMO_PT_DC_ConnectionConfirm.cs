...
namespace SaiTer.ATE.Business
{
    ...
    /// 连接确认测试（目前依照便携式来开发）
    /// </summary>
    public class CHAdeMO_PT_DC_ConnectionConfirm : BusinessBase
    {
        public CHAdeMO_PT_DC_ConnectionConfirm(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        double ConnectSingle = AllEquipStateData.DicBMS_JP_DC_StateData[item].ProxDetect;
                        dic.Add(item, ConnectSingle.ToString());
                    }


                    ProcessDataTmp(dic, "充电机插枪时", "连接信号", "1", "1");
                }
            }
