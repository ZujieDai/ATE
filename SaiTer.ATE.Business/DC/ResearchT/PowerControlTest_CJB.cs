...
namespace SaiTer.ATE.Business
{
    ...
    /// 研测：功率控制（恒功率段）
    /// </summary>
    public class PowerControlTest_CJB : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public PowerControlTest_CJB(int type)
        {
            TrialType = type;
        }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    for (int k = 0; k < 3; k++)
                    {
                        ...
                        Voltage[1] = (MaxAllowChargeVoltage + MinAllowChargeVoltage) / 2 > ModuleMinimumVoltage ? 
                            (MaxAllowChargeVoltage + MinAllowChargeVoltage) / 2 : ModuleMinimumVoltage;
                        Voltage[2] = MinAllowChargeVoltage < ModuleMinimumVoltage ? ModuleMinimumVoltage : MinAllowChargeVoltage;
            ...
            for (int i = 0; i < Voltage.Length; i++)
            {
                if (IsRLoad(Voltage[i], Current[i]))
                {
                    ...
                    while(timeout-- > 0)
                    {
                        ...
                        if (Math.Abs((outputPower - OutputPower_New) / OutputPower_New * 100) < 判定准则)
                            break;
                        Thread.Sleep(1000);
                    }
                    timeout = 100;
