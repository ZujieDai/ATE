...
namespace SaiTer.ATE.Business
{
    ...
    /// </summary>
    public class GB34657_ManualInspection : BusinessBase
    {
        string itemFlow = "";
        public GB34657_ManualInspection(int type) { TrialType = type; }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    string info = $"【{itemFlow}】供电设备采用的充电模式应符合GB/T 18487.1-2015中5.1规定的对应的电动汽车充电模式使用条件\r\n注：勾选上为PASS，否则为FAIL";
                    CountDownTimeInfo(info, CheckTime, 2);
                    ProcessData();

                    itemFlow = "连接方式C";
                    info = $"【{itemFlow}】充电机应为连接方式C(含连接方式C下的电缆组件)\r\n注：勾选上为PASS，否则为FAIL";
                    CountDownTimeInfo(info, CheckTime, 2);
                    ProcessData();

                    //itemFlow = "交流充电桩检查";
                    //info = $"【{itemFlow}】交流充电桩应为连接方式A或连接方式B或连接方式C(含连接方式C下的电缆组件)";
                    //CountDownTimeInfo(info, CheckTime, 2);
                    //ProcessData();

                    itemFlow = "缆上控制与保护装装置应为连接方式B";
                    info = $"【{itemFlow}】缆上控制与保护装装置应为连接方式B(带有功能盒的电缆组件)\r\n注：勾选上为PASS，否则为FAIL";
                    CountDownTimeInfo(info, CheckTime, 2);
                    ProcessData();
