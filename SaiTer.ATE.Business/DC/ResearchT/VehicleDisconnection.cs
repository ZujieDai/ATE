...
{
    /// <summary>
    /// 研发测试:连接检测信号断开试验/车辆接口断开测试/开关S断开测试
    /// </summary>

    public class VehicleDisconnection : BusinessBase
    {
        int trlTimeOut_S = 30;
        /// <summary>
        /// 需求电压
        ...
        public VehicleDisconnection(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    //SendNoticeToUIAndTxtFile("关闭电子负载中...");
                    //Thread.Sleep(200);
                    ...
                    {
                        double K3K4Value = System.Math.Abs(Convert.ToDouble(OscillographInstrumentReadValue(2, false, 0)));
                        if (Ta == 100 && K3K4Value < 1)
                        {
                    ...
                    double CurrentTime = GetTriggerTime_Single(1, false, 0, 5, 1, false, false, 0.05);//0到100ms
