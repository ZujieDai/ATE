...
namespace SaiTer.ATE.Business
{
    ...
    /// 研发测试:最大恒功率
    /// </summary> 
    public class MaximumConstantPower : BusinessBase
    {
        ...
        int 恒功率段数;
        double 最大充电电压, 最小充电电压, 恒功率段最大充电电流;

        public MaximumConstantPower(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    Voltage[1] = (MaxAllowChargeVoltage + ModuleMinimumVoltage) / 2;
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        {
                            string info = $"最大恒功率试验{i + 1}，需求电压{Voltage[i]}V电流{Current[i]}A";
                            ...
                            {
                                volt = Voltage[i] + 20;
                                ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, volt, Current[2], false, 390);
                            }
                            else
                            {
                                ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, Voltage[i], Current[2], false, 390);
                            }
                            ...
                            WaitDCCurrentWithTime(testWorkParam.lstIDs, Current[i] + 20, 50);
                            ...
                            ProcessDataTmp(dicPower, info, "充电功率(KW)", (MaxOutputPower * 0.99).ToString(), "-");
                            //}
                            //else
                            //{
                            //if (i == 2)
