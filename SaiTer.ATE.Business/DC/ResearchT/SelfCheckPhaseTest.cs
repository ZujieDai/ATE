...
namespace SaiTer.ATE.Business
{
    ...
    /// 自检阶段测试--总
    /// </summary>
    public class SelfCheckPhaseTest : BusinessBase
    {
        private int SelfCheckPhaseType;
        private string StatusName = "";
        public SelfCheckPhaseTest(int trialType) { TrialType = trialType; }
        private double HandshakeUpper = 1050;    //握手电压超上限
        private double HandshakeNoraml = 500;   //握手电压正常
        private double HandshakeLower = 150;   //握手电压超下限
        ...
        private void TrialMethod(int type)
        {
            ...
            try
            {
                ...
                Thread.Sleep(1000 * 3);
                //超上限则选择低值(正常电池电压（报文最高允许充电电压范围内）)
                if (type == 1)
                {
                    dic = ControlEquipMent.Oscillograph.Oscillograph_ReadMeasure("MAXimum", 4, false, 0);
                    ProcessDataTmp(dic, StatusName, "充电电压(V)", "-", MaxAllowChargeVoltage <= HandshakeUpper ? (MaxAllowChargeVoltage * 1.01).ToString() : (HandshakeUpper * 1.01).ToString());
                }
