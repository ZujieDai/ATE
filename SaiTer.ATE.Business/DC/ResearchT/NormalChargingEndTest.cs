...
namespace SaiTer.ATE.Business
{
    ...
    /// 正常充电结束测试
    /// </summary>
    public class NormalChargingEndTest : BusinessBase
    {
        int trlTimeOut_S = 30;
        double DemandVoltage = 500;
        double DemandCurrent = 30;

        public NormalChargingEndTest(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    string s2Value = ControlEquipMent.Oscillograph.GetChannelValue(7, false, 0).FirstOrDefault().Value;
                    ...
                    double CST_SOC = GetData_Value(15, true, 15, 0.999);
                    ProcessDataIsNor(CH15_14.ToString(), dImgs, "CST车中止报文", "1", "被动终止充电");
                    //ProcessDataIsNor(CST_SOC.ToString(), dImgs, "CST-SOC中止报文", "1");
                    ...
                    if (Customer != null && Customer.ToString().Contains("ZD"))
                    {
                        ...
                        ProcessDataTmp(dic, "被动终止充电", "充电电流(A)", "-", "-");
                    }
                    ...
                    ProcessDataTmp(dTime, "被动终止充电", "停充时间差(ms)", "0", "1000", dImgs);

                    CountDownTimeInfo("请确认车辆插头电子锁应能正确解锁!\r\n注：勾选上为可以正确解锁", 999, 2);
                    ProcessDataConnect("被动终止充电");
                    #endregion
                    //清空BST通道的数值
                    ControlEquipMent.Oscillograph?.Oscillograph_SetCanChild_Open(15, 3, false);
                    Thread.Sleep(1000);
                    ControlEquipMent.Oscillograph?.Oscillograph_SetCanChild_Open(15, 3, true);
                    ...
                    s2Value = ControlEquipMent.Oscillograph.GetChannelValue(7, false, 0).FirstOrDefault().Value;
                    K1K2s2 = System.Math.Abs(Convert.ToDouble(s2Value));
                    //if (K1K2s2 < 2)
                    //{
                    //    OscillographInstrument_SetTrigger(6, 7, 0, "RISE", false, 50);
                    //}
                    ...
                    //    GetTriggerTime_Single(7, false, 0, 6, 1, false, false, -0.05);
                    ControlEquipMent.Oscillograph?.Oscillograph_CursorPosition_X_Single(1, false, 0, 0, true);
                    time = GetTriggerTime_Single(8, false, 0, DownVolt, 0, false, false, -0.05, true);
