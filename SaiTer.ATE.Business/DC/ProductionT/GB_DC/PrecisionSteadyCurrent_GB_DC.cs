...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流  稳流精度
    /// </summary>
    public class PrecisionSteadyCurrent_GB_DC : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public PrecisionSteadyCurrent_GB_DC(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                //{
                //    for (int c = 0; c < BMSCurrents.Count; c++)
                //    {
                //        Dictionary<int, string> dic = new Dictionary<int, string>();
                //        TrialMethod(dic, BMSVolts[v], 0.85 + InputVoltDiff, BMSCurrents[c], "输入电压85%额定值");
                //        TrialMethod(dic, BMSVolts[v], 1.0, BMSCurrents[c], "输入电压100%额定值");
                //        TrialMethod(dic, BMSVolts[v], 1.15 - InputVoltDiff, BMSCurrents[c], "输入电压115%额定值");
                //    }
                //}
        ...
        private void TrialMethod(double BMSDemandVolt, double rate, double current, bool isMid)
        {
            ...
            double bmsVolt = BMSDemandVolt;
            if (ControlEquipMent.ResistanceLoad == null)
                bmsVolt = BMSDemandVolt + 20 >= MaxAllowChargeVoltage ? MaxAllowChargeVoltage : BMSDemandVolt + 20;
            ...
            if (BMSDemandVolt + 20 >= MaxAllowChargeVoltage)
                SetLoadPara(testWorkParam.lstIDs, BMSDemandVolt - 10, current + 20, BMSDemandVolt - 5, current);
            else
