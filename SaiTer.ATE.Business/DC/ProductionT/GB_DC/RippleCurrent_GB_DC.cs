...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流_电流纹波
    /// </summary>
    public class RippleCurrent_GB_DC : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public RippleCurrent_GB_DC(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            //BMS需求电压最小值(V)=400|BMS需求电压最大值(V)=745|负载电流设置(A)=40|判定准则(±%)=0.5|电流纹波最大值
            //BMSDemandVolt_Min = Convert.ToDouble(strParams[0].Split('=')[1]);
            //BMSDemandVolt_Max = Convert.ToDouble(strParams[1].Split('=')[1]);
            //LoadCurrent = Convert.ToDouble(strParams[2].Split('=')[1]);
            //BMSDemandVolt = LstChargerInfo[0].NominalVoltage;
            //判定准则 = Convert.ToDouble(strParams[3].Split('=')[1]);
            //CurrentPKPK = Convert.ToDouble(strParams[4].Split('=')[1]);
