...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流  输入电流过冲
    /// </summary>
    public class InputCurrentOvershoot_GB_DC : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public InputCurrentOvershoot_GB_DC(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    dicResultData.Add(1, (ErrorValue - 100).ToString("F2"));
                    ProcessDataTmp(dicResultData, "输入电流过冲", "过冲值(%)", "-", errorPecent.ToString("F2"));
