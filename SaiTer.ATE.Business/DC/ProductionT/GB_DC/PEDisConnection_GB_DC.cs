...
{
    /// <summary>
    /// 保护接地连续性测试_国标直流（PE断线测试）
    /// </summary>
    public class PEDisConnection_GB_DC : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public PEDisConnection_GB_DC(int trialType) { TrialType = trialType; }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                //ACUpTime(testWorkParam.lstIDs, 1, LstChargerInfo[0].NominalVoltage - 30, 1);//AC回路动作时间

                //Dictionary<int, double> values = new Dictionary<int, double>();
                //values.Add(LstChargerInfo[0].ChargerId, 0.5);
                //ControlEquipMent.Oscilloscope.Oscilloscope_CursorPosition_X_Single(testWorkParam.lstIDs, 1, values,false);
                //ACDownTime(testWorkParam.lstIDs, 1, 50, 2);//AC回路动作时间
