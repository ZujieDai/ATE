...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流 过温保护试验
    /// </summary>
    public class OverTempProtection_GB_DC : BusinessBase
    {
        ...
        public OverTempProtection_GB_DC(int trialType) { TrialType = trialType; }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                SetLoadPara(testWorkParam.lstIDs, BMSDemandVolt - 20, ResiLoadCurrent / 2, BMSDemandVolt, ResiLoadCurrent / 2);
                Thread.Sleep(1000);
                SetLoadDCON(testWorkParam.lstIDs);
                WaitDCCurrent(testWorkParam.lstIDs, ResiLoadCurrent / 2);
                Thread.Sleep(1000);
                ...
                {
                    double volt = AllEquipStateData.DicPowerAnalyzer_StateData.FirstOrDefault().Value.Channel4RMSVolt;
                    int timeout = 30;
                    while(timeout-- > 0)
                    {
                        if(volt < 50 || volt > MaxAllowChargeVoltage)
                        {
                            volt = AllEquipStateData.DicPowerAnalyzer_StateData.FirstOrDefault().Value.Channel4RMSVolt;
                            Thread.Sleep(100);
                            continue;
                        }
                        break;
                    ...
                    double current = AllEquipStateData.DicPowerAnalyzer_StateData.FirstOrDefault().Value.Channel4RMSCurrent;
                    timeout = 30;
                    while (timeout-- > 0)
                    {
                        if (current < (ResiLoadCurrent / 2) * 0.8 || current > (ResiLoadCurrent / 2) * 1.2)
                        {
                            current = AllEquipStateData.DicPowerAnalyzer_StateData.FirstOrDefault().Value.Channel4RMSCurrent;
                            Thread.Sleep(100);
                            continue;
                        }
                        break;
                    ...
                    //while (timeout-- > 0)
                    //{
                    //    if (data < (ResiLoadCurrent / 2) * 200 / 1000 * 0.8 || data > (ResiLoadCurrent / 2) * MaxAllowChargeVoltage / 1000 * 1.2)
                    //    {
                    //        data = AllEquipStateData.DicPowerAnalyzer_StateData.FirstOrDefault().Value.Channel4Power;
                    //        Thread.Sleep(100);
                    //        continue;
                    ...
                    //    break;
                    //}
                    d3.Add(item, (volt * current / 1000.0).ToString("F2"));
                }
                ProcessDataTmp(d1, "正常充电状态", "充电电压(V)", "-", "-");
                ProcessDataTmp(d2, "正常充电状态", "充电电流(A)", "-", "-");
                ...
                ProcessDataTmp(d1, "过温保护状态", "充电电压(V)", "-", "-");
