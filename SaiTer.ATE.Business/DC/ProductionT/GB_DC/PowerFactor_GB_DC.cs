...
{
    /// <summary>
    /// 国标直流  功率因数   效率试验  效率功率因数
    /// </summary>
    public class PowerFactor_GB_DC : BusinessBase
    {
        double ErrorValue1 = 0.95;
        double ErrorValue2 = 0.98;
        double ErrorValue3 = 88;
        double ErrorValue4 = 93;
        int trlTimeOut_S = 30;
        ...
        public PowerFactor_GB_DC(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            //功率因数下限(实际功率/额定功率≤50%)=0.95|功率因数下限(实际功率/额定功率>50%)=0.98|效率值下限(实际功率/额定功率≤50%)=88|效率值下限(实际功率/额定功率>50%)=93
            if (strParams.Length > 1 && strParams[0].IndexOf('=') > -1)
            {
                ErrorValue1 = double.Parse(strParams[0].Split('=')[1]);
                ErrorValue2 = double.Parse(strParams[1].Split('=')[1]);
                ErrorValue3 = double.Parse(strParams[2].Split('=')[1]);
                ErrorValue4 = double.Parse(strParams[3].Split('=')[1]);
            }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    Current[5] = MaxAllowChargeCurrent;
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        if (i == 4 || i == 6)
                        {
                            ...
                            SendNoticeToUIAndTxtFile(string.Format("设置负载需求电压为{0},充电电流为{1}", Voltage[i], Current[i]));
                        ...
                        else
                        {
                            ...
                            if (Current[i] >= MaxAllowChargeCurrent)
                                SetLoadPara(testWorkParam.lstIDs, Voltage[i] - 20, Current[i] - 5 , Voltage[i], Current[i]);
                            else
                                SetLoadPara(testWorkParam.lstIDs, Voltage[i] - 20, Current[i], Voltage[i], Current[i]);
                        ...
                        for (int j = 0; j < 5; j++)
                        {
                            ...
                            {
                                if (efficiency < 93 || efficiency >= 100)
                                {
                                    Thread.Sleep(1000);
                                    efficiency = AllEquipStateData.DicPowerAnalyzer_StateData[LstChargerInfo[0].ChargerId].Efficiency;
                                }
                        ...
                        powerfactor = AllEquipStateData.DicPowerAnalyzer_StateData[LstChargerInfo[0].ChargerId].TotalPowerFactor;//功率因数
                        for (int j = 0; j < 5; j++)
                        {
                        ...
                        if(powerfactor <= 0)
                        {
                            ...
                            for (int j = 0; j < 5; j++)
                            {
                                ...
                                {
                                    Thread.Sleep(1000);
                                    powerfactor = AllEquipStateData.DicPowerAnalyzer_StateData[LstChargerInfo[0].ChargerId].Channel1PowerFactor;
                                }
                                else
                                {
                                    break;
                        ...
                        if (Rate <= 0.5)
                        {
                            ...
                            ProcessDataTmp(dic, "20%≤P0/PN≤50%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,效率值", ErrorValue3.ToString("f0"), "100");
                            dic.Clear();
                            dic.Add(LstChargerInfo[0].ChargerId, powerfactor.ToString("F3"));
                            ProcessDataTmp(dic, "20%≤P0/PN≤50%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,功率因数", ErrorValue1.ToString("f2"), "1");
                        }
                        else
                        {
                            dic.Add(LstChargerInfo[0].ChargerId, efficiency.ToString("F3"));
                            ProcessDataTmp(dic, "50%＜P0/PN≤100%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,效率值", ErrorValue4.ToString("f0"), "100");
                            dic.Clear();
                            dic.Add(LstChargerInfo[0].ChargerId, powerfactor.ToString("F3"));
                            ProcessDataTmp(dic, "50%＜P0/PN≤100%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,功率因数", ErrorValue2.ToString("f2"), "1");
                        }
