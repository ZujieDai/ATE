...
namespace SaiTer.ATE.Business
{
    ...
    /// </summary>
    public class PowerFactor_GB_DCR : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public PowerFactor_GB_DCR(int type)
        {
            TrialType = type;
        }
                        ...
                        if(IsRLoad(Voltage[i],Current[i]))
                        {
                            ...
                            if (i == 4 || i == 6)
                            {
                                ...
                                if (ControlEquipMent.FeedbackLoad != null)
                                    SendNoticeToUIAndTxtFile(string.Format("设置负载需求电压为{0},充电电流为{1}", Voltage[i] - 5, Current[i] - 5));
                                else if (ControlEquipMent.ResistanceLoad != null)
                                    SendNoticeToUIAndTxtFile(string.Format("设置负载需求电压为{0},充电电流为{1}", Voltage[i] - 20, Current[i] - 5));
                                SetLoadPara(testWorkParam.lstIDs, Voltage[i] - 5, Current[i] - 5, Voltage[i] - 20, Current[i] - 5);
                                Thread.Sleep(800);
                            ...
                            if (Rate <= 0.5)
                            {
                                ...
                                ProcessDataTmp(dic, "20%≤P0/PN≤50%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,效率值", "88", "100");
                                dic.Clear();
                                dic.Add(LstChargerInfo[0].ChargerId, powerfactor.ToString("F3"));
                                ProcessDataTmp(dic, "20%≤P0/PN≤50%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,功率因数", "0.95", "1");
                            }
                            else
                            {
                                dic.Add(LstChargerInfo[0].ChargerId, efficiency.ToString("F3"));
                                ProcessDataTmp(dic, "50%＜P0/PN≤100%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,效率值", "93", "100");
                                dic.Clear();
                                dic.Add(LstChargerInfo[0].ChargerId, powerfactor.ToString("F3"));
                                ProcessDataTmp(dic, "50%＜P0/PN≤100%", "电压" + Voltage[i] + "V,电流" + Current[i] + "A,功率因数", "0.98", "1");
                            }
                            SetLoadDCOFF(testWorkParam.lstIDs);
                            Thread.Sleep(300);
                            //SetLoadDCOFF(testWorkParam.lstIDs);
