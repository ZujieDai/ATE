...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流    绝缘检测功能试验
    /// </summary>
    public class InsulationDetection_GB_DC : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public InsulationDetection_GB_DC(int type)
        {
            TrialType = type;
        }
        ...
        public override void ExecuteMethod()
        {
            ...
            finally
            {
                ...
                if (BMSDemandVolt < 390 && !(Customer != null && Customer.Contains("DH")))
                    ControlEquipMent.BMS.BMSSetKState_DC(lstIDs, 1000, BMSDemandVolt - 10, kstate.ToArray());
                else
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    kstate[26] = true;
                    if (BMSDemandVolt < 390 && !(Customer != null && Customer.Contains("DH")))
                        ControlEquipMent.BMS.BMSSetKState_DC(testWorkParam.lstIDs, 1000, BMSDemandVolt - 10, kstate.ToArray());
                    else
                    ...
                    while (timeout-- > 0)
                    {
                        ...
                        if (bmsData.Count() < 1 || bmsData.Values.FirstOrDefault() == null)
                            continue;
                        if (ChangeBMSChargeStatus(bmsData.First().Value.ChargingState) > 4 || ChangeBMSChargeStatus(bmsData.First().Value.ChargingState) < 1)
                        {
                            break;
                        }
                    ...
                    if (BMSDemandVolt < 390 && !(Customer != null && Customer.Contains("DH")))
                    {
                        ControlEquipMent.BMS.BMSSetBatteryVoltage(testWorkParam.lstIDs, BMSDemandVolt);
                        //Thread.Sleep(200);
                        //ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, BMSDemandVolt + 10, MaxAllowChargeVoltage, MaxAllowChargeCurrent);
                    }
                    else
                    ...
                    if (insulationStrs != null)
                    {
                        ...
                        for(int i = insulationTexts.Length - 1; i >= 0; i--)
                        {
                            if (double.TryParse(insulationTexts[i], out double insulationValue)) 
                            {
                                if (insulationValue * 1000 / BMSDemandVolt <= 500 * 0.95 && insulationValue * 1000 / BMSDemandVolt >= 100 * 1.05)
                                {
                                    KStatus1 = i + 1;
                                    KStatus2 = i + 9;
                                    resistanceVal = $"{insulationTexts[i]}KΩ";
                                    break;
                                }
                            }
                        }
                        if(KStatus1 == 0)
                        {
                            ProcessDataResult("绝缘检测异常", $"额定电压{BMSDemandVolt}V", "未找到合适档位", EmTrialResult.NA);
                            return;
                        }
                    }
                    else if (isCharger2015)
                    {
                    ...
                    if (insulationStrs != null)
                    {
                        ...
                        for (int i = insulationTexts.Length - 1; i >= 0; i--)
                        {
                            if (double.TryParse(insulationTexts[i], out double insulationValue))
                            {
                                if (insulationValue * 1000 / BMSDemandVolt <= 100 * 1.05)
                                {
                                    KStatus1 = i + 1;
                                    KStatus2 = i + 9;
                                    resistanceVal = $"{insulationTexts[i]}KΩ";
                    ...
                    if (BMSDemandVolt < 390 && !(Customer != null && Customer.Contains("DH")))
                    {
                        ...
                        ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, 390, LstChargerInfo[0].NominalVoltage, MaxAllowChargeCurrent);
                    Thread.Sleep(200);
                    ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, LstChargerInfo[0].NominalVoltage, MaxAllowChargeCurrent, true, LstChargerInfo[0].NominalVoltage);
                    Thread.Sleep(200);
        ...
        private void TrialMethod(string sState, string ItemName1, string ItemName2, List<bool> lstKState, bool CanCharge = true)
        {
            ...
            if (BMSDemandVolt < 390 && !(Customer != null && Customer.Contains("DH")))
            {
                ControlEquipMent.BMS.BMSSetBatteryVoltage(testWorkParam.lstIDs, BMSDemandVolt - 10);
                Thread.Sleep(200);
                ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, BMSDemandVolt, MaxAllowChargeVoltage, MaxAllowChargeCurrent);
            }
            else
                ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, 390, LstChargerInfo[0].NominalVoltage, MaxAllowChargeCurrent);
            Thread.Sleep(200);
            ...
            if (BMSDemandVolt < 390 && !(Customer != null && Customer.Contains("DH")))
                ControlEquipMent.BMS.BMSSetKState_DC(testWorkParam.lstIDs, 1000, BMSDemandVolt - 10, lstKState.ToArray());
            else
