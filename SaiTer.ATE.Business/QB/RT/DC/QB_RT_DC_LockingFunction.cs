...
namespace SaiTer.ATE.Business
{
    ...
    /// 企标研测直流：锁止功能试验
    /// </summary>
    public class QB_RT_DC_LockingFunction : BusinessBase
    {
        string ItemFlow = "";
        double BMSDemandVolt = 0;
        double ResiLoadCurrent = 0;
        float trlTimeOut_S = 100;//超时时间
        public QB_RT_DC_LockingFunction(int type) { TrialType = type; }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                ItemFlow = "模拟电子锁未可靠锁止";
                CountDownTimeInfo("请模拟电子锁未可靠锁止，然后点击确认按钮", 100, 0);
                CountDownTimeInfo("请确认供电设备应停止充电或不启动充电且有报警\r\n勾选上代表供电设备停止充电或不启动充电且有报警", 20, 2);
                ProcessData();
