...
namespace SaiTer.ATE.Business
{
    ...
    /// 企标直流：保护接地连续性试验(录波板)
    /// </summary>
    public class QB_RT_DC_GroundingContinuity_WaveRecoder : BusinessBase
    {
        int trlTimeOut_S = 30;
        /// <summary>
        /// 需求电压
        ...
        public QB_RT_DC_GroundingContinuity_WaveRecoder(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_ReadChannelData(testWorkParam.lstIDs, 4, ref CH_CC2, "CC2");
                    ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_ReadChannelData(testWorkParam.lstIDs, 8, ref CH_K1K2, "K1K2");
                    DataAnalysis_WaveRecoder.GetDCSingleTime(CH_CC2, true, 5, ref Time_CC2);
                    ...
                    ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_SetCursor(testWorkParam.lstIDs, 1, Time_CC2);//设置光标1
                    ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_SetCursor(testWorkParam.lstIDs, 2, Time_K1K2);//设置光标2
                    double Time_Stop = Math.Abs(Time_CC2 - Time_K1K2);
