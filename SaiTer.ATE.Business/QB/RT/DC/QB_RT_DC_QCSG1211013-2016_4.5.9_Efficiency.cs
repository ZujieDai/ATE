...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流  效率试验(恒功率)
    /// </summary>
    public class QB_RT_DC_Efficiency : BusinessBase
    {
        int trlTimeOut_S = 30;
        double RealPower = 0;
        double Rate = 0;
        ...
        public QB_RT_DC_Efficiency(int type)
        {
            TrialType = type;
        }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (Customer != null && Customer.Contains("ZD"))
                    {
                        for (int i = 0; i < Voltage.Length; i++)
                        {
                            TrialMethod(Voltage[i], Current[i], false);
                        }
                    }
    ...
    /// 企标研测直流：效率试验
    /// </summary>
    public class QB_RT_DC_Efficiency_Old : BusinessBase
    {
        int trlTimeOut_S = 30;
        double RealPower = 0;
        double Rate = 0;
        ...
        public QB_RT_DC_Efficiency_Old(int type)
        {
            TrialType = type;
        }
        ...
        private void TrialMethod(double voltage, double current, bool type)
        {
            //if (IsRLoad(voltage, current))
            {
                ...
                if (type)
                {
                    if (current == MaxAllowChargeCurrent)
                    {
                        if (ControlEquipMent.FeedbackLoad != null || ControlEquipMent.LoopFeedbackLoad != null)
                            ...
                            SendNoticeToUIAndTxtFile(string.Format("设置负载需求电压为{0}，充电电流为{1}", voltage - 5, current));
                        SetLoadPara(testWorkParam.lstIDs, voltage - 20, current - 5, voltage - 5, current);
                        CheckCurrent = current - 5;
                    }
                    else
                    {
                ...
                else
                {
                    if (current == MaxAllowChargeVoltage)
                    {
                        if (ControlEquipMent.FeedbackLoad != null || ControlEquipMent.LoopFeedbackLoad != null)
                            ...
                            SendNoticeToUIAndTxtFile(string.Format("设置负载需求电压为{0}，充电电流为{1}", voltage, current - 5));
                        SetLoadPara(testWorkParam.lstIDs, voltage - 20, current - 5, voltage, current - 5);
                        CheckCurrent = current - 5;
                    }
                    else
                    {
                        if (ControlEquipMent.FeedbackLoad != null || ControlEquipMent.LoopFeedbackLoad != null)
                            SendNoticeToUIAndTxtFile(string.Format("设置负载需求电压为{0}，充电电流为{1}", voltage - 20, current + 20));
                        else if (ControlEquipMent.ResistanceLoad != null)
                            SendNoticeToUIAndTxtFile(string.Format("设置负载需求电压为{0}，充电电流为{1}", voltage, current - 5));
                        SetLoadPara(testWorkParam.lstIDs, voltage, current + 20, voltage, current - 5);
                        CheckCurrent = current;
                    }
                }
                Thread.Sleep(300);
                ...
                for (int j = 0; j < 5; j++)
                {
                    if (efficiency < 89 || efficiency >= 100)
                    {
                        Thread.Sleep(1000);
                        efficiency = AllEquipStateData.DicPowerAnalyzer_StateData[LstChargerInfo[0].ChargerId].Efficiency;
                        RealPower = AllEquipStateData.DicPowerAnalyzer_StateData[LstChargerInfo[0].ChargerId].Channel4Power;
                ...
                if (Rate <= 0.5)
                {
                    ...
                    dic.Clear();
                    dic.Add(LstChargerInfo[0].ChargerId, efficiency.ToString("F3"));
                    ProcessDataTmp(dic, itemFlow, "电压" + voltage + "V,电流" + current + "A,效率值", "89", "100");
                }
                else
                {
                    dic.Add(LstChargerInfo[0].ChargerId, totalPower.ToString("F3"));
