...
namespace SaiTer.ATE.Business
{
    ...
    /// 企标研测直流：输入电流过冲
    /// </summary>
    public class QB_RT_DC_InputImpulseCurrent : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public QB_RT_DC_InputImpulseCurrent(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    ControlEquipMent.Oscilloscope.Oscilloscope_StorageDepth(testWorkParam.lstIDs, "100k");//2M
                    //设置时基ms
                    ControlEquipMent.Oscilloscope.Oscilloscope_TimeBase(testWorkParam.lstIDs, true, "400", "0.4");//这里放大时基，有些桩功率释放缓慢，导致检测不到过冲
                    //添加测量值
                    ...
                    ControlEquipMent.Oscilloscope.Oscilloscope_Trigger(testWorkParam.lstIDs, 0, "RISE", "DC", "EDGE", "10", 2, ((ResiLoadCurrent - 10) * 0.65).ToString("F2"), "Single");//上升边沿触发
                    Thread.Sleep(300);
                    if (Customer.Equals("XJ"))
                    {
                        SetLoadPara(testWorkParam.lstIDs, BMSDemandVolt - 20, ResiLoadCurrent - 10, BMSDemandVolt, ResiLoadCurrent - 10);
                    }
                    else
                    {
                    ...
                    Thread.Sleep(300);
                    ControlEquipMent.Oscilloscope.Oscilloscope_IsRun(testWorkParam.lstIDs, true);   //tek示波器设置触发模式不会自动开始滚动
                    Thread.Sleep(1000 * 5);
    ...
    /// 企标研测直流：输入电流过冲
    /// </summary>
    public class QB_RT_DC_InputImpulseCurrent_Old : BusinessBase
    {
        Dictionary<int, string> dImgs = new Dictionary<int, string>();//图片存储
        int trlTimeOut_S = 30;
        ...
        double errorPecent = 10;//误差百分比
        public QB_RT_DC_InputImpulseCurrent_Old(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            //误差百分比=10.00|测试电压(V)=750.00|测试电流(A)=100.00
            dImgs = new Dictionary<int, string>();//图片存储
            string[] strParams = TrialItem.ResultParams.Split('|');
            //BMSDemandVolt = Convert.ToDouble(strParams[0].Split('=')[1]);
            //LoadCurrent = Convert.ToDouble(strParams[1].Split('=')[1]);
