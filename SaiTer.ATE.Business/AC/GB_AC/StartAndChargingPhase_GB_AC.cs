...
    /// 国标交流启动和充电阶段测试
    /// </summary>
    public class StartAndChargingPhase_GB_AC : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public StartAndChargingPhase_GB_AC(int trialType) { TrialType = trialType; }
        ...
        public override void ExecuteMethod()
        {
            ...
            finally
            {
                ...
                //2026/4/27--DZJ 方便连续测试，后面紧结充电结束测试，不模拟插拔
                //SetCPReresh();
...
                if (testWorkParam.lstIDs.Count <= 0)
                ...
                try
                {
                    ...
                    //设置触发
                    ControlEquipMent.Oscilloscope.Oscilloscope_Trigger(testWorkParam.lstIDs, 0, "FALL", "DC", "EDGE", "10", 3, "0", "Auto");//上升边沿触发，自动
                    ...
                    //}
                    Data_Tmp = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "TOP", 3);//高值
                    ProcessDataTmp(Data_Tmp, "状态2’", "CP正电压(V)", "8.2", "9.8", dImgs);
                    //调整示波器位置
                    ...
                    //ProcessDataTmp(dd, "状态2’", "CP上升时间(us)", "-", "2");
                    ProcessDataTmp(dd, "状态2’", "CP上升时间(us)", "0", CPPWMUpMax.ToString());
                    ...
                    ProcessDataTmp(dd, "状态2’", "CP下降时间(us)", "0", CPPWMDownMax.ToString(), dImgs);
                    ...
                    //Thread.Sleep(1000);
                    Data_Tmp = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "TOP", 3);//高值
                    ProcessDataTmp(Data_Tmp, "状态3’", "CP正电压(V)", "5.2", "6.8", dImgs);
                    ...
                    ProcessDataTmp(dd, "状态3’", "CP上升时间(us)", "0", CPPWMUpMax.ToString());
                    ...
                    ProcessDataTmp(dd, "状态3’", "CP下降时间(us)", "0", CPPWMDownMax.ToString(), dImgs);
                    //恢复滤波功能
                    lstConditionState[15] = false;
                    ControlEquipMent.ControlBoard.ControlResistanceSetRelay(lstConditionState);
                }
