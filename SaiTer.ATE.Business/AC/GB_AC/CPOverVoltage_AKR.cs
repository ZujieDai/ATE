...
{
    /// <summary>
    /// 控制导引电压超限（江苏AKR交流目前的检测流程,要求按照占空比去判断是否停充）
    /// </summary>
    public class CPOverVoltage_AKR : BusinessBase
    {
        ...
        int num = 1;
        double 默认占空比 = 0.533;
        public CPOverVoltage_AKR(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            占空比误差范围 = double.Parse(strParams[10].Split('=')[1]);



            占空比下限 = (默认占空比 - 占空比误差范围 / 100) * 100;
            占空比上限 = (默认占空比 + 占空比误差范围 / 100) * 100;
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                SendNoticeToUIAndTxtFile("关闭交流源");
                ControlEquipMent.ACSource.ACSource_OFF(testWorkParam.lstIDs);
                Thread.Sleep(3000);
                SendNoticeToUIAndTxtFile("打开交流源");
                ...
                Thread.Sleep(2000);
                ItemFlow = "上偏超限测占空比" + "|R2电阻 = " + 超限上限值R2 + "，R3电阻 = " + 超限上限值R3;
               
                占空比下限 = (默认占空比 - 占空比误差范围 / 100) * 100;
                占空比上限 = (默认占空比 + 占空比误差范围 / 100) * 100;
                ProcessData();
                ItemFlow = "R2电阻 = " + 超限上限值R2 + "，R3电阻 = " + 超限上限值R3;
                Dictionary<int, string> dic = new Dictionary<int, string>();
                for (int i = 0; i < testWorkParam.lstIDs.Count; i++)
                {
                    double volt = AllEquipStateData.DicBMS_AC_StateData[testWorkParam.lstIDs[i]].PhaseA_Voltage;
                    dic.Add(testWorkParam.lstIDs[i], volt.ToString());
                }
                ProcessDataTmp(dic, "上偏超限测充电电压", ItemFlow, "0", "80");



                SendNoticeToUIAndTxtFile("恢复R2、R3阻值并刷新CP信号模拟拔/插枪");
                ControlEquipMent.BMS.BMS_SetResistance(testWorkParam.lstIDs, 限值内R2, 限值内R3);
                Thread.Sleep(100);
                ControlEquipMent.BMS.BMS_OFF(testWorkParam.lstIDs);
                Thread.Sleep(1000);
        ...
        public override void ProcessData()
        {
            foreach (var item in testWorkParam.lstIDs)
            {
                ...
                double dutyCycle = AllEquipStateData.DicBMS_AC_StateData[testWorkParam.lstIDs[i]].CPDutyCycle;

                if (dutyCycle <= 占空比上限 && dutyCycle >= 占空比下限)
                {
                ...
                LstTrialData[k].ExtentData = ItemFlow + "|" + 占空比下限 + " | " + 占空比上限 + " | " + dutyCycle.ToString("F2");
