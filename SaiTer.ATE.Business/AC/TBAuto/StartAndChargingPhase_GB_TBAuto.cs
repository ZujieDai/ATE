...
    /// 国标交流启动和充电阶段测试
    /// </summary>
    public class StartAndChargingPhase_GB_TBAuto : BusinessBase
    {
        private static bool isFirstTest = true;
        Dictionary<int, string> Data_Tmp = new Dictionary<int, string>();//临时测试数据
        bool IsCardCharg = false;   //是否为刷卡充电

        double CPDutyMin, CPDutyMax, CPFreqError, CPVoltError1, CPVoltError2, CPVoltError3, CPVoltErrorNegative;
        ...
        public StartAndChargingPhase_GB_TBAuto(int trialType) { TrialType = trialType; }
        ...
        public override void InitializeParams()
        {
            try
            {
                ...
                //占空比下限(%)=2.00|占空比上限(%)=96.00|负向电平误差(V)=0.6|频率误差(Hz)=30|状态1正向电平误差(V)=0.6|状态2正向电平误差(V)=0.6|状态3正向电平误差(V)=0.53|状态2'上升时间上限(μs)=10|状态2'下降时间上限(μs)=13|
                ...
                CPFreqError = Convert.ToDouble(strParams[3].Split('=')[1]);
                CPVoltError1 = Convert.ToDouble(strParams[4].Split('=')[1]);
                CPVoltError2 = Convert.ToDouble(strParams[5].Split('=')[1]);
                CPVoltError3 = Convert.ToDouble(strParams[6].Split('=')[1]);
                CPPWMUpMax2 = Convert.ToDouble(strParams[7].Split('=')[1]);
                CPPWMDownMax2 = Convert.ToDouble(strParams[8].Split('=')[1]);
                CPPWMUpMax3 = Convert.ToDouble(strParams[9].Split('=')[1]);
                CPPWMDownMax3 = Convert.ToDouble(strParams[10].Split('=')[1]);
                OutputVoltError = Convert.ToDouble(strParams[11].Split('=')[1]);
                IsCardCharg = double.Parse(strParams[12].Split('=')[1]) == 1;
                if (strParams.Length > 13)
                {
                    Chage2Sleep = (int)Convert.ToDouble(strParams[13].Split('=')[1]);
                    StandbyPowerSleep = (int)Convert.ToDouble(strParams[14].Split('=')[1]);
                    StandbyPowerMax = Convert.ToDouble(strParams[15].Split('=')[1]);
                }
            }
            catch (Exception ex)
            {
...
                                LstTrialData[i].ExtentData = DateTime.Now.ToString()
                ...
                try
                {
                    ...
                    if (isFirstTest)
                    {
                    	...
                    	Thread.Sleep(sleepTime);
                    	ControlEquipMent.Oscilloscope.Oscilloscope_Trigger(testWorkParam.lstIDs, 0, "RISE", "DC", "EDGE", "40", 3, "0", "Auto");
                    	Thread.Sleep(sleepTime);
                        isFirstTest = false;
                    }
                    ...
                    for (int i = 0; i < testWorkParam.lstIDs.Count; i++)
                    {
                        ...
                        if (volt <= v * 0.95 || volt >= v * 1.05)
                        {
                            //电表读的是输入电压，必须判断已进入充电状态
                            var dd1 = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "TOP", 3);
                            if (Convert.ToInt32(dd1.First().Value) >= 5.2 && Convert.ToInt32(dd1.First().Value) <= 6.8)
                            {
                                dd1 = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "FREQ", 3);
                                if (Convert.ToInt32(dd1.First().Value) >= 970 && Convert.ToInt32(dd1.First().Value) <= 1030)
                                    volt = ControlEquipMent.ElectricMeter.EM_GetVolt(testWorkParam.lstIDs).First().Value.First();
                            }
                        }
