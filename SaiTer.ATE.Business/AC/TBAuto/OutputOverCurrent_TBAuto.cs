...
namespace SaiTer.ATE.Business
{
    ...
    /// 输出过流测试
    /// </summary>
    public class OutputOverCurrent_TBAuto : BusinessBase
    {
        ...
        double outputVoltMin1 = 220, outputVoltMax1 = 240, outputVoltMin2 = 0, outputVoltMax2 = 20;
        public OutputOverCurrent_TBAuto(int type)
        {
            TrialType = type;
        }
        ...
        {
            Init();
            //输出过流值(A)=19.00|正常电流值(A)=16.00|桩类型（1为刷卡桩，否则为0）=0.00|不动作电流输出电压下限(V)=220|不动作电流输出电压上限(V)=240|过流保护输出电压下限(V)=0|过流保护输出电压上限(V)=20
            ...
            outputVoltMin1 = Convert.ToDouble(strParams[3].Split('=')[1]);
            outputVoltMax1 = Convert.ToDouble(strParams[4].Split('=')[1]);
            outputVoltMin2 = Convert.ToDouble(strParams[5].Split('=')[1]);
            outputVoltMax2 = Convert.ToDouble(strParams[6].Split('=')[1]);
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                ProcessDataTmp(Data_Tmp, "不动作电流", "输出电压(V)", outputVoltMin1.ToString(), outputVoltMax1.ToString());
                ...
                ProcessDataTmp(Data_Tmp, "过流保护", "输出电压(V)", outputVoltMin2.ToString(), outputVoltMax2.ToString());
