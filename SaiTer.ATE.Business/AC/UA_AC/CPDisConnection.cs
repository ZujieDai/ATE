...
namespace SaiTer.ATE.Business
{
    ...
    /// CP断线测试
    /// </summary>
    public class CPDisConnection : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public CPDisConnection(int trialType) { TrialType = trialType; }
        ...
        public override void InitializeParams()
        {
            ...
            if (ItemParams.Length > 0 && ItemParams[0].Split('=').Length == 2)
            {
                sleepTime = Convert.ToInt32(ItemParams[0].Split('=')[1]);
            }
            //断开时间(ms)=3000|保护后最大电压(V)=20
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                if (testWorkParam.lstIDs.Count > 0)
                {
                    ...
                    ProcessDataTmp(dd, "CP断线", "S1开关切换时间(ms)", "0", maxValue, dImgs);
                    ...
                    ProcessDataTmp(Data_Tmp, "CP断线", "断线后输出电压(V)", "0", MaxVolt.ToString());
