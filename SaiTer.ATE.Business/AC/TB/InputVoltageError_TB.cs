...
namespace SaiTer.ATE.Business
{
    ...
    /// </summary>
    public class InputVoltageError_TB : BusinessBase
    {
        ...
        public InputVoltageError_TB(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                if (TrialType == (int)EmTrialType.输入过压保护及恢复测试 || TrialType == (int)EmTrialType.输入过压保护测试)
                {
                    ...
                    ProcessDataTmp(Data_Tmp, "输入过压", "输出电压(V)", "0", "30");
                }
                else
                {
                    ProcessDataTmp(dd, "输入欠压", "输入电压(V)", "-", "-");
                    ProcessDataTmp(Data_Tmp, "输入欠压", "输出电压(V)", "0", "30");
                }
                    ...
                    || TrialType == (int)EmTrialType.输入欠压保护及恢复测试)
                {
                        ...
                        for (int j = 0; j < RecoveryTime * 5; j++)
                        {
                            ...
                            else
                            {
                                ...
                                if (inputVoltage > NormalVoltage * 0.9 && inputVoltage < NormalVoltage * 1.05)
                                    if (voltage > NormalVoltage - 5 && voltage < NormalVoltage + 5)
                                        break;
                            }
                        }
                    ...
                    if (TrialType == (int)EmTrialType.输入过压保护及恢复测试)
                    {
                        ...
                        ProcessDataTmp(Data_Tmp, "过压恢复", "输出电压(V)", (NormalVoltage - 5).ToString("F2"), (NormalVoltage + 5).ToString("F2"));
                    }
                    else
                    {
                        ProcessDataTmp(dd, "欠压恢复", "输入电压(V)", "-", "-");
                        ProcessDataTmp(Data_Tmp, "欠压恢复", "输出电压(V)", (NormalVoltage - 5).ToString("F2"), (NormalVoltage + 5).ToString("F2"));
                    }
                }
