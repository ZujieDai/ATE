...
namespace SaiTer.ATE.Business
{
    ...
    /// 枪头RC电阻测试   欧标
    /// </summary>
    public class RcResistance_EU_AC : BusinessBase
    {
        double diff = 0;
        double maxp = 1.1;
        ...
        double CloseResistance1 = 1500;
        double CloseResistance2 = 680;
        double CloseResistance3 = 220;
        double CloseResistance4 = 100;
        double OpenResistance1 = 3300;
        double OpenResistance2 = 3380;
        double OpenResistance3 = 3520;
        double OpenResistance4 = 3400;

        double MinOpenResistance, MaxOpenResistance, MinCloseResistance, MaxCloseResistance;

        bool isHaveLocker = true;

        public RcResistance_EU_AC(int type)
        {
            TrialType = type;
        }
        public override void InitializeParams()
        {
            ...
            if (LstChargerInfo[0].NominalCurrent <= 10)
            {
                CloseResistance = 1500;//3300
                OpenResistance = 3300;
            }
            else if (LstChargerInfo[0].NominalCurrent <= 16)
            {
                CloseResistance = 680;//3380
                OpenResistance = 3380;
            }
            else if (LstChargerInfo[0].NominalCurrent <= 32)
            {
                CloseResistance = 220;//3520
                OpenResistance = 3520;
            }
            else if (LstChargerInfo[0].NominalCurrent <= 63)
            {
                CloseResistance = 100;//3400
                OpenResistance = 3400;
            }
            ...
            //是否具备机械锁（0是不具备，1是具备）=0|偏移值=-30
            string[] strItems = TrialItem.ItemParams.Split('|');
            if (strItems.Length > 0 && strItems[0].Split('=').Length > 1)
            {
                isHaveLocker = Convert.ToInt32(double.Parse(strItems[0].Split('=')[1])) == 1;
            }
            if (strItems.Length > 1)
            {
                diff = double.Parse(strItems[1].Split('=')[1]);
            }
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                if (isHaveLocker)
                {
                    ...
                    {
                        double CCResistance = AllEquipStateData.DicBMS_AC_StateData[item].CCResistance + diff;
                        dic.Add(item, CCResistance.ToString("F2"));
                    }
                    ProcessDataTmp(dic, "枪头RC电阻", "按下机械锁", MinOpenResistance.ToString(), MaxOpenResistance.ToString());//解锁状态
                ...
                ProcessDataTmp(dic, "枪头RC电阻", isHaveLocker ? "松开机械锁" : "电阻值(Ω)", MinCloseResistance.ToString(), MaxCloseResistance.ToString());//锁止状态
