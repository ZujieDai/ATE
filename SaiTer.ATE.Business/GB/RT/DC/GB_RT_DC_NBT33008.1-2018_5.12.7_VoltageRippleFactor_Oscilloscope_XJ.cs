...
{
    /// <summary>
    /// 国标研测直流：电压纹波因数试验（示波器）(XJ专用)
    /// </summary>
    public class GB_RT_DC_VoltageRippleFactor_Oscilloscope_XJ : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public GB_RT_DC_VoltageRippleFactor_Oscilloscope_XJ(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                for (int v = 0; v < Test_Voltages.Count; v++)
                {
                    ProcessDataResult(testWorkParam.lstIDs, "-", Test_Voltages[v].ToString(), true, $"设定充电机输出电压--{Test_Voltages[v].ToString()}");
                    for (int a = 0; a < Test_ACSourceRates.Count; a++)
                    {
                        for (int c = 0; c < Test_Currents.Count; c++)
                        {
                            TrialMethod(Test_Voltages[v], Test_Currents.Max(), Test_ACSourceRates[a], Test_Currents[c]); ;
                        }
                    }
                }
