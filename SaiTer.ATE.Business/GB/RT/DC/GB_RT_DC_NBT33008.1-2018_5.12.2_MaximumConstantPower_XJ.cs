...
namespace SaiTer.ATE.Business
{
    ...
    /// 研测:最大恒功率（恒功率段）(XJ专用)
    /// </summary> 
    public class GB_RT_DC_MaximumConstantPower_XJ : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public GB_RT_DC_MaximumConstantPower_XJ(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            判定准则 = ConvertStringToDouble(GetDicValue(strItemParams, "判定准则(%)", "99"))/100;
            MaxPower = ConvertStringToDouble(GetDicValue(strItemParams, "最大功率(kW)", MaxOutputPower.ToString()));
            string stmpU = null;
            string stmpI = null;
            double dtmpU = 0;
            double dtmpI = 0;
            for (int i = 0; i < 15; i++)
            {
                ...
                if (stmpU != null )
                {
                    ...
                    dtmpI = Math.Round(MaxPower * 1000 / dtmpU, 2);
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    TrialMethon(Test_Voltages.ToArray(), Test_Currents.ToArray());
        ...
        private void TrialMethon(double[] Voltage, double[] Current)
        {
            ...
            for (int i = 0; i < Voltage.Length; i++)
            {
                ...
                {
                    if (Customer.Equals("XJ")//XJ客户要求
                        && MaxPower * 1000 - 50 > Voltage[i] * Current[i])//不符合恒功率的点不做
                    {
                        continue;
                    }
                    ...
                    {
                        if (!Customer.Equals("XJ"))//XJ客户要求按照实际来
                        {
                            volt = Voltage[i] + 20;
                        }
                    ...
                    if (Customer.Equals("XJ"))//XJ现场稳定时间要加一点
                    {
                        Thread.Sleep(5 * 1000);
                    }
                    ...
                    foreach (var item in testWorkParam.lstIDs)
                    {
                        ...
                        d1.Add(item, MaxPower.ToString("F2"));
                    }
                    ...
                    if (MaxPower * 1000 > Voltage[i] * Current[i])
                        ProcessDataTmp(dicPower, info, "充电功率(kW)", "-", "-");
                    else
                        ProcessDataTmp(dicPower, info, "充电功率(kW)", (MaxPower * 判定准则).ToString(), "-");
                    //}
                    //else
                    //{
                    //if (i == 2)
