...
namespace SaiTer.ATE.Business
{
    ...
    /// 输出电压设定误差（XJ专用）
    /// </summary>
    public class GB_RT_DC_OutputVoltageSetError_XJ : BusinessBase
    {
        int trlTimeOut_S = 30;
        double Error = 0.5;
        ...
        public GB_RT_DC_OutputVoltageSetError_XJ(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            Error = ConvertStringToDouble(GetDicValue(strItemParams, "判定标准(%)", "0.5"));
            ResiLoadCurrent = ConvertStringToDouble(GetDicValue(strItemParams, "输出电流(A)", ResiLoadCurrent.ToString()));
            string stmpU = null;
            string stmpI = null;
            double dtmpU = 0;
            double dtmpI = 0;
            for (int i = 0; i < 15; i++)
            {
                ...
                if (stmpU != null)
                {
                    if (Convert.ToDouble(stmpU) == 0 )
                    {
                        continue;
                    }
                    dtmpU = ConvertStringToDouble(stmpU);
                    Test_Voltages.Add(dtmpU);

                    dtmpI = ResiLoadCurrent;
                    dtmpI = dtmpI > MaxAllowChargeCurrent ? MaxAllowChargeCurrent : dtmpI;
                    Test_Currents.Add(ResiLoadCurrent);
                }
            }
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                double[] BMSVolts = Test_Voltages.ToArray(); 
                List<string> sStates = new List<string>();
                foreach (var vol in BMSVolts)
                {
                    sStates.Add($"输出电压点={vol}V");
                }

                ProcessDataResult(testWorkParam.lstIDs, "-", $"{ResiLoadCurrent}", true, $"设定输出电流点(A) —— " + ResiLoadCurrent.ToString());
                for (int v = 0; v < BMSVolts.Length; v++)
                {
                    ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, BMSVolts[v], MaxAllowChargeCurrent, true, BMSVolts[v]);
                    ProcessDataResult(testWorkParam.lstIDs, BMSVolts[v].ToString(), "BMS需求电压(V)", true, sStates[v]);
                    Thread.Sleep(500);

                    //double current = RatedCurrent / 2;
                    double current = ResiLoadCurrent;
