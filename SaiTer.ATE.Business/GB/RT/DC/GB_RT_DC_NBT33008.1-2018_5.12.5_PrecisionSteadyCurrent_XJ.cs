...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流  稳流精度(XJ专用)
    /// </summary>
    public class GB_RT_DC_PrecisionSteadyCurrent_XJ : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        List<double> Test_ACSourceRates = new List<double>() { 1, 0.85, 1.15 };
        List<double> Test_Voltages = new List<double>();
        List<double> Test_Currents = new List<double>();

        public GB_RT_DC_PrecisionSteadyCurrent_XJ(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            判定准则 = ConvertStringToDouble(GetDicValue(strItemParams, "判定准则(±%)", "1"));
            //Test_ACSourceRates[0] = ConvertStringToDouble(GetDicValue(strItemParams, "输入电压值正常(%)", "100")) / 100;
            Test_ACSourceRates[1] = ConvertStringToDouble(GetDicValue(strItemParams, "输入电压值低(%)", "85")) / 100;
            Test_ACSourceRates[2] = ConvertStringToDouble(GetDicValue(strItemParams, "输入电压值高(%)", "115")) / 100;
            string stmpU = null;
            string stmpI = null;
            for (int i = 0; i < 5; i++)
            {
                ...
                if (stmpU != null && stmpI != null)
                {
                    if (Convert.ToDouble(stmpU) == 0 || Convert.ToDouble(stmpI) == 0)
                    {
                        continue;
                    }
                    Test_Voltages.Add(ConvertStringToDouble(stmpU));
                    Test_Currents.Add(ConvertStringToDouble(stmpI));
                }
            }
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                for (int v = 0; v < Test_Currents.Count; v++)
                {
                    ProcessDataResult(testWorkParam.lstIDs, "-", Test_Currents[v].ToString(), true, $"设定充电机输出电流--{Test_Currents[v].ToString()}");
                    for (int a = 0; a < Test_ACSourceRates.Count; a++)
                    {
                        for (int c = 0; c < Test_Voltages.Count; c++)
                        {
                            //交流源是100%并且输出电流为中间电流的时候记录参考值
                            bool isMid = a == 0 && c == 0;
                            TrialMethod(Test_Voltages[c], Test_ACSourceRates[a], Test_Currents[v], isMid);
                        }
                    }
                }
        ...
        private void TrialMethod(double BMSDemandVolt, double rate, double current, bool isMid)
        {
            ...
            if (!BMSInfo.Contains("充电中"))
            {
                if (!CheckSwipingCard(testWorkParam.lstIDs))
                {
                    ProcessDataResult(testWorkParam.lstIDs, "-", "无法启动充电", false, $"设置BMS电压{LstChargerInfo[0].NominalVoltage.ToString()}V,电流250A");
                    return;
                }
            }
            ...
            if (BMSDemandVolt + 20 >= MaxAllowChargeVoltage)
                SetLoadPara(testWorkParam.lstIDs, BMSDemandVolt - 10, current + 20, BMSDemandVolt - 10, current);
            else
