...
namespace SaiTer.ATE.Business
{
    ...
    /// 正常充电结束测试(录波板)
    /// </summary>
    public class GB_RT_DC_NormalChargingEndTest_WaveRecoder : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public GB_RT_DC_NormalChargingEndTest_WaveRecoder(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    SendNoticeToUIAndTxtFile("等待电流稳定中...");
                    Thread.Sleep(3000);
                    Thread.Sleep(10 * 1000);
                    ...
                    if (Customer.Equals("XJ"))
                    {
                        CountDownTimeInfo("请手动控制桩停止充电再点击确认或者是。", 30, 1);
                    }
                    else
                    {
                        CountDownTimeInfo("请手动控制桩停止充电再点击确认或者是。", 99999, 1);
