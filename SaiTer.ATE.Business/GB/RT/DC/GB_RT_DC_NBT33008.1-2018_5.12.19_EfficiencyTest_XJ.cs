...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标直流  效率试验(恒功率)(XJ专用)
    /// </summary>
    public class GB_RT_DC_EfficiencyTest_XJ : BusinessBase
    {
        int trlTimeOut_S = 30;
        double RealPower = 0;
        double Rate = 0;
        ...
        public GB_RT_DC_EfficiencyTest_XJ(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            判定准则1 = ConvertStringToDouble(GetDicValue(strItemParams, "判定准则1(%)", "88"));
            判定准则2 = ConvertStringToDouble(GetDicValue(strItemParams, "判定准则2(%)", "93"));
            string stmpU = null;
            string stmpI = null;
            for (int i = 0; i < 15; i++)
            {
        ...
        private void TrialMethod(double voltage, double current, bool type)
        {
            //if (IsRLoad(voltage, current))
            {
                ...
                if (Rate <= 0.5)
                {
                    ...
                    ProcessDataTmp(dic, itemFlow, "电压" + voltage + "V,电流" + current + "A,效率值", 判定准则1.ToString(), "100");
                }
                else
                {
                    dic.Add(LstChargerInfo[0].ChargerId, totalPower.ToString("F3"));
                    ...
                    dic.Clear();
                    dic.Add(LstChargerInfo[0].ChargerId, efficiency.ToString("F3"));
                    ProcessDataTmp(dic, itemFlow, "电压" + voltage + "V,电流" + current + "A,效率值", 判定准则2.ToString(), "100");
                }
                SetLoadDCOFF(testWorkParam.lstIDs);
                Thread.Sleep(300);
                //SetLoadDCOFF(testWorkParam.lstIDs);
