...
{
    /// <summary>
    /// 研发测试:测量值更新时间(录波板)   //GB_RT_DC_NBT33008.1-2018_5.12.18_MeasurementUpdate_WaveRecoder
    /// </summary>
    public class GB_RT_DC_MeasurementUpdate_WaveRecoder : BusinessBase
    {
        int trlTimeOut_S = 30;
        /// <summary>
        /// 需求电压
        ...
        public GB_RT_DC_MeasurementUpdate_WaveRecoder(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitEquiMent()
        {
            ...
            SendNoticeToUIAndTxtFile("初始化录波板配置中...");
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    WaveData CH_CCS_Current = new WaveData();
                    ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_ReadDigitalChannelData(testWorkParam.lstIDs, 138, 0, ref CH_CCS_Current, "CCSCurrent");
                    double dCCSValue = DataAnalysis_WaveRecoder.GetWavePointVave(CH_CCS_Current, CH_CCS_Current.LinePoints_Y.Count - 2);


                    string CCSValue = dCCSValue.ToString();


                    Dictionary<int, string> DCCSValue = new Dictionary<int, string>();
                    foreach (var item in testWorkParam.lstIDs)
                    {
                        DCCSValue.Add(item, CCSValue);
                    }

                    ProcessDataTmp(DCCSValue, "正常状态", "CCS桩报文电流值(A)", "-", "-");
                    ...
                    //等待电流启动
                    WaitDCCurrentChangeWithTime(testWorkParam.lstIDs, DemandCurrent, ChangeCurrent, 35);
                    System.Threading.Thread.Sleep(2000);
                    ...
                    //读取录波板数据
                    double Time_CCS_Current = 0;
                    double Time_OutputCurrent = 0;
                    CH_CCS_Current = new WaveData();
                    ...
                    DataAnalysis_WaveRecoder.GetDCSingleTime(CH_CCS_Current, true, (DemandCurrent + ChangeCurrent) / 2, ref Time_CCS_Current);
                    DataAnalysis_WaveRecoder.GetDCSingleTime_M(CH_CCS_Current, false, (DemandCurrent + ChangeCurrent) / 2, ref Time_CCS_Current, Time_CCS_Current);
                    DataAnalysis_WaveRecoder.GetDCSingleTime(CH_OutputCurrent, false, (DemandCurrent + ChangeCurrent) / 2, ref Time_OutputCurrent);

                    ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_SetCursor(testWorkParam.lstIDs, 1, Time_CCS_Current);//设置光标1
                    ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_SetCursor(testWorkParam.lstIDs, 2, Time_OutputCurrent);//设置光标2
                    double Time_Response = Math.Abs(Time_CCS_Current - Time_OutputCurrent);
                    ...
                    string CSSValue2 = DataAnalysis_WaveRecoder.GetWavePointVave(CH_CCS_Current, CH_CCS_Current.LinePoints_Y.Count - 2).ToString();
                    ...
                    double Current_Message = System.Math.Abs(Convert.ToDouble(CSSValue2));
                    ...
                    foreach (var item in testWorkParam.lstIDs)
                    {
                        DTime.Add(item, (Time_Response).ToString());
                    }
