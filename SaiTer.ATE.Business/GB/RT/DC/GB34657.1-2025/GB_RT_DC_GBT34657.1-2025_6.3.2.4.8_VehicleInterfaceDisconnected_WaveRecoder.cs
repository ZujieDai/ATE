...
namespace SaiTer.ATE.Business
{
    /// <summary>
    /// 车辆接口断开测试（录波版）
    /// 标准：GB/T 34657.1-2017  6.3.4.8
    /// 功能：模拟充电前/充电中车辆接口断开，验证保护、状态、时序、报文
    /// </summary>
    public class GB_RT_DC_2025_VehicleInterfaceDisconnected_WaveRecoder : BusinessBase
    {
        #region 常量
        ...
        #region 构造函数
        public GB_RT_DC_2025_VehicleInterfaceDisconnected_WaveRecoder(int type)
        {
            TrialType = type;
        }
        #endregion
        ...
        private void ExecuteTestItems(List<int> chargerIds)
        {
            try
            {
                ...
                // 4. 执行两个场景测试
                RunAbnormalTest(chargerIds, 390, 500, "充电前");
                RunAbnormalTest(chargerIds, 390, 500, "充电中");
            }
            catch (Exception ex)
            {
                SendException(ex);
        ...
        /// 执行车辆接口断开异常测试（含录波）
        /// </summary>
        private void RunAbnormalTest(List<int> chargerIds, double batteryVoltage, double reportVoltage, string testScene)
        {
            ...
            string testTitle = $"车辆接口断开测试({testScene})";
