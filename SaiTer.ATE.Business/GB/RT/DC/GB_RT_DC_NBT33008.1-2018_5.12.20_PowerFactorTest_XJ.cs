...
{
    /// <summary>
    /// 国标直流  功率因数试验(恒功率)(XJ专用)
    /// </summary>
    public class GB_RT_DC_PowerFactorTest_XJ : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public GB_RT_DC_PowerFactorTest_XJ(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            判定准则1 = ConvertStringToDouble(GetDicValue(strItemParams, "判定准则1", "0.95"));
            判定准则2 = ConvertStringToDouble(GetDicValue(strItemParams, "判定准则2", "0.98"));
            string stmpU = null;
            string stmpI = null;
            for (int i = 0; i < 15; i++)
            {
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    for (int i = 0; i < Test_Voltages.Count; i++)
                    {
                        TrialMethod(Test_Voltages[i], Test_Currents[i], true);
                    }
        ...
        private void TrialMethod(double voltage, double current, bool type)
        {
            //if (IsRLoad(voltage, current))
            {
                ...
                if (Rate <= 0.5)
                {
                    ...
                    ProcessDataTmp(dic, itemFlow, "电压" + voltage + "V,电流" + current + "A,功率因数", 判定准则1.ToString(), "1");
                }
                else
                {
                    string itemFlow = "50%＜P0/PN≤100%" + chargerType;
                    dic.Add(LstChargerInfo[0].ChargerId, powerfactor.ToString("F3"));
                    ProcessDataTmp(dic, itemFlow, "电压" + voltage + "V,电流" + current + "A,功率因数", 判定准则2.ToString(), "1");
                }
                SetLoadDCOFF(testWorkParam.lstIDs);
                Thread.Sleep(300);
                //SetLoadDCOFF(testWorkParam.lstIDs);
