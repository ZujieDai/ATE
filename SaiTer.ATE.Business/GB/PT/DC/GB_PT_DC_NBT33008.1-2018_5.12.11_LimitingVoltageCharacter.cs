...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标产测直流：限压特性试验
    /// </summary>
    public class GB_PT_DC_LimitingVoltageCharacter : BusinessBase
    {
        int trlTimeOut_S = 30;
        /// <summary>
        /// 需求电压
        ...
        public GB_PT_DC_LimitingVoltageCharacter(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    { DemandVoltage - 220, DemandVoltage, DemandVoltage, DemandVoltage, DemandVoltage - 260 };
                    List<double> Currents = new List<double>()
                    { DemandCurrent + 20,  DemandCurrent - 15, DemandCurrent - 40, DemandCurrent - 10, DemandCurrent + 20 };
                    ...
                    for (int i = 0; i < Voltages.Count; i++)
                    {
                        ...
                        Thread.Sleep(100);
                        WaitDCCurrentWithTime(testWorkParam.lstIDs, resistLoadCurrent, 60);
                        Thread.Sleep(3000);
                        ...
                        {
                            double DCVoltage = AllEquipStateData.DicPowerAnalyzer_StateData != null ? AllEquipStateData.DicPowerAnalyzer_StateData[item].Channel4RMSVolt : AllEquipStateData.DicBMS_DC_StateData[item].ChargingVoltage;
                            while (time-- > 0)
                            {
                                if (DCVoltage < judgeVolt * 0.95 || DCVoltage > judgeVolt * 1.05)
                                {
                                    DCVoltage = AllEquipStateData.DicPowerAnalyzer_StateData != null ? AllEquipStateData.DicPowerAnalyzer_StateData[item].Channel4RMSVolt : AllEquipStateData.DicBMS_DC_StateData[item].ChargingVoltage;
                                    Thread.Sleep(100);
                                }
                                else
                                    break;
                        ...
                        foreach (var item in testWorkParam.lstIDs)
                        {
                            double DCCurrent = AllEquipStateData.DicPowerAnalyzer_StateData != null ? AllEquipStateData.DicPowerAnalyzer_StateData[item].Channel4RMSCurrent : AllEquipStateData.DicBMS_DC_StateData[item].ChargingCurrent;
                            while (time-- > 0)
                            {
                                if (DCCurrent < judgeCurrent * 0.95 || DCCurrent > judgeCurrent * 1.05)
                                {
                                    DCCurrent = AllEquipStateData.DicPowerAnalyzer_StateData != null ? AllEquipStateData.DicPowerAnalyzer_StateData[item].Channel4RMSCurrent : AllEquipStateData.DicBMS_DC_StateData[item].ChargingCurrent;
                                    Thread.Sleep(100);
                                }
                                else
                                    break;
