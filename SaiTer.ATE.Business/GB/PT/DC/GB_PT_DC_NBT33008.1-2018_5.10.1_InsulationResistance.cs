...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标产测直流：绝缘电阻试验（安规）
    /// </summary>
    public class GB_PT_DC_InsulationResistance : BusinessBase
    {
        string FlowItemName = "";
        ...
        public GB_PT_DC_InsulationResistance(int trialType)
        {
            TrialType = trialType;
        }
        /// <summary>
        ...
        public override void ExecuteMethod()
        {
            ...
            {
                //保存试验结果
                ControlEquipMent.Safety.SafetySetParam(lstIDs, "FUNC:TEST OFF", "\r\n", "\r\n");
                SaveTrialResult();
                    ...
                    for (int i = 0; i < LstTrialData.Count; i++)
                    {
                        if (LstTrialData[i].IsCheck)
                        {
                            if (LstTrialData[i].TrialResult == EmTrialResult.Wait)
                            {
                                ...
                                LstTrialData[i].ExtentData = TrialItem.ItemName + "|测试超时|-|" + HISETResistance + "|null";
        ...
        public override void ProcessData()
        {
            ...
            try
            {
                ...
                {
                    string data = TrialItem.ItemName + "|" + FlowItemName + "|" + LOSETResistance + "|" + HISETResistance + "|null";
                    StResultData Result = ResultData.Dequeue();
                    ...
                    {
                        if (Result.LstData[0].ToString().IndexOf("TEST") >= 0)
                            continue;
                        LstTrialData[k].Data1 = Result.LstData[0].ToString();
                        string[] ReturnStrS = Result.LstData[0].ToString().Split(',');
                        if (ReturnStrS.Length >= 5)
                        {
                            data = TrialItem.ItemName + "|" + FlowItemName + "|" + LOSETResistance + "|" + HISETResistance + "|" + ReturnStrS[3].Replace("ohm", "Ω");
