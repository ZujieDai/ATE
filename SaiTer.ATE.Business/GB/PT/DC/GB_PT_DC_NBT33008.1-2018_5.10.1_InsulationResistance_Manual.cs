...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标产测直流：绝缘电阻试验_人工（安规）
    /// </summary>
    public class GB_PT_DC_InsulationResistance_Manual : BusinessBase
    {
        string FlowItemName = "";
        ...
        public GB_PT_DC_InsulationResistance_Manual(int trialType)
        {
            TrialType = trialType;
        }
        /// <summary>
        ...
        private void StarTestItem()
        {
            ...
            {
                ProcessDataResult(testWorkParam.lstIDs, LOSETResistance, HISETResistance, InputData, FlowItemName, false);
            }
            else
            {
                ProcessDataResult(testWorkParam.lstIDs, LOSETResistance, HISETResistance, InputData, FlowItemName, true);
            }
        }
                    ...
                    for (int i = 0; i < LstTrialData.Count; i++)
                    {
                        if (LstTrialData[i].IsCheck)
                        {
                            if (LstTrialData[i].TrialResult == EmTrialResult.Wait)
                            {
                                ...
                                LstTrialData[i].ExtentData = TrialItem.ItemName + "|测试超时|"+ LOSETResistance +"| " + HISETResistance + "|null";
