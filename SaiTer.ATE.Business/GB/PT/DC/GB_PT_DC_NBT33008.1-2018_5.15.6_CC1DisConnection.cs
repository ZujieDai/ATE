...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标产测直流：连接检测信号断开试验
    /// </summary>
    public class GB_PT_DC_CC1DisConnection : BusinessBase
    {
        int CC1KSIndex = -1;    //CC1采集开关(程控板索引号)
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public GB_PT_DC_CC1DisConnection(int trialType) { TrialType = trialType; }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                if (CC1KSIndex > -1)
                {
                    ...
                    if (CC1CHNum > -1)
                    {
                        ControlEquipMent.Oscilloscope.Oscilloscope_Channel_Set(testWorkParam.lstIDs, CC1CHNum, true, "DC", "20", "1", "CC1", "1", "V", false, "2", "0");
                        Thread.Sleep(waitTime);
                    }
