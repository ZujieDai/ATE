...
namespace SaiTer.ATE.Business
{
    ...
    /// 自检阶段测试(录波板)
    /// </summary>
    public class GB_PT_DC_SelfCheckPhaseTest_WaveRecoder : BusinessBase
    {
        ...
        public GB_PT_DC_SelfCheckPhaseTest_WaveRecoder(int trialType) { TrialType = trialType; }
        ...
        private void TrialMethod(int type)
        {
            ...
            try
            {
                ...
                SendNoticeToUIAndTxtFile("等待数据读取中...");
                timeout = 30;
                while (timeout-- > 0)
                {
                    Thread.Sleep(1000);
                ...
                ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_ReadDigitalChannelData(testWorkParam.lstIDs, 2, 1, ref CH_BHM, "BHM");
                ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_ReadChannelData(testWorkParam.lstIDs, 6, ref CH_K1K2State, "K1K2State");
                ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_ReadDigitalChannelData(testWorkParam.lstIDs, 4, 1, ref CH_CRM, "CRM");
                ControlEquipMent.WaveRecoderCtrl?.WaveRecoder_ReadDigitalChannelData(testWorkParam.lstIDs, 6, 1, ref CH_BRM, "BRM");
                ...
                if (type == 0)
                {
                    ...
                    dic.Clear();
                    dic.Add(testWorkParam.lstIDs[0], (Time_K1K2State * 1000).ToString());
                    ProcessDataTmp(dic, StatusName, "闭合K1和K2进行绝缘检测时间点(ms)", (Time_BHM * 1000).ToString(), "-");


                    SendNoticeToUIAndTxtFile("读取CRM报文时间点...");
                    double Time_CRM = 0;//CRM报文时间点
                    DataAnalysis_WaveRecoder.GetCANMsgTime(CH_CRM, 0.5, true, ref Time_CRM);
                    dic.Clear();
                    dic.Add(testWorkParam.lstIDs[0], (Time_CRM * 1000).ToString());
                    ProcessDataTmp(dic, StatusName, "CRM报文时间点(ms)", (Time_K1K2State * 1000).ToString(), "-");
                    ...
                    DataAnalysis_WaveRecoder.GetCANMsgTime(CH_BRM, 100, true, ref Time_BRM);
                    dic.Clear();
                    dic.Add(testWorkParam.lstIDs[0], (Time_BRM * 1000).ToString());
                    ProcessDataTmp(dic, StatusName, "多包标识报文PGN(ms)", (Time_CRM * 1000).ToString(), "-");
