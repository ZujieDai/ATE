...
namespace SaiTer.ATE.Business
{
    ...
    /// 预充电功能试验(新录波板)
    /// </summary>
    public class GB_PT_DC_PreCharging_WaveRecoder : BusinessBase
    {
        int trlTimeOut_S = 30;
        /// <summary>
        /// 需求电压
        ...
        public GB_PT_DC_PreCharging_WaveRecoder(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    ControlEquipMent.WaveRecoderCtrl.WaveRecoder_ReadDigitalChannelData(testWorkParam.lstIDs, 23, 0, ref CH_BCPVoltage, "BCPVoltage");//读取数字通道数据（BCP电池电压）
                    //DataAnalysis_WaveRecoder.GetBCPBatteryVolt(wdtmp, ref Y1);
                    Y1 = DataAnalysis_WaveRecoder.GetNotZeroValue_first(CH_BCPVoltage);
                    ...
                    DataAnalysis_WaveRecoder.GetPreChargeVolt(CH_OutputVoltage, Y1 - 10, ref Y2);
                    double Dvalue = 0;
                    Dvalue = Y1 - Y2;
                    ...
                    ProcessDataTmp(dY1Y2, "闭合K5和K6后", "差值", " 1 ", "10", dImgs);
