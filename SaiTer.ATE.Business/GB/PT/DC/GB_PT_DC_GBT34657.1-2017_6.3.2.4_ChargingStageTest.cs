...
namespace SaiTer.ATE.Business
{
    ...
    /// 充电阶段测试
    /// </summary>
    public class GB_PT_DC_ChargingStageTest : BusinessBase
    {
        string ItemFlow = "";
        int trlTimeOut_S = 30;
        ...
        public GB_PT_DC_ChargingStageTest(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, DemandVoltage + 10, DemandCurrent, true, 390);
                    //ControlEquipMent.BMS.BMS_ON(testWorkParam.lstIDs);

                    SendNoticeToUIAndTxtFile("启动充电中");
                    SystemEvent.SendWaitSwipingCard(testWorkParam.lstIDs, DemandVoltage + 10, LstChargerInfo[0].ChargerType, 0);

                    SendNoticeToUIAndTxtFile("开启负载中...");
                    SetLoadPara(testWorkParam.lstIDs, DemandVoltage - 10, DemandCurrent + 10, DemandVoltage, DemandCurrent);
        ...
        public override void ProcessData()
        {
            foreach (var item in DicManualVerifyResult)
            {
                ...
                else
                {
                    ...
                    var dImages = ControlEquipMent.WaveRecoderCtrl?.WaveRecoderSaveScreen(testWorkParam.lstIDs);
                    ...
                    string sVolt = AllEquipStateData.DicBMS_DC_StateData[testWorkParam.lstIDs[i]].ChargingVoltage.ToString();
                    string passValueMin = (DemandVoltage * 0.95).ToString();
                    string passValueMax = (DemandVoltage * 1.05).ToString();
                    if (CanCharge)
                    {
                        if (Convert.ToDouble(sVolt) < Convert.ToDouble(passValueMin)
                            || Convert.ToDouble(sVolt) > Convert.ToDouble(passValueMax))
                        {
                            LstTrialData[k].TrialResult = EmTrialResult.Fail;
                            LstTrialData[k].ExtentData = ItemFlow + "|充电电压|" + passValueMin + "|" + passValueMax + "|" + sVolt + "|" + dImages[1];
                        }
                        else
                        {
                            LstTrialData[k].TrialResult = EmTrialResult.Pass;
