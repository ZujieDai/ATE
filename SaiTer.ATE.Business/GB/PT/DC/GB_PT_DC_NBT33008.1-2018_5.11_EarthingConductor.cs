...
namespace SaiTer.ATE.Business
{
    ...
    /// 国标产测直流：接地试验（安规）
    /// </summary>
    public class GB_PT_DC_EarthingConductor : BusinessBase
    {
        ...
        public GB_PT_DC_EarthingConductor(int trialType)
        {
            TrialType = trialType;
        }
        /// <summary>
        ...
        {
            //GB电流值(A)=25.00|HISET电阻值(mΩ)=150.00|LOSET电阻值(mΩ)=0.00|测试时间(S)=1.00|斜坡时间(S)=0.50|GB补偿值(mΩ)=0.01|测试频率(Hz)=50.00
        ...
        public override void InitEquiMent()
        {
            ...
            list[8] = true;
            list[index] = true;
            info = "闭合KM4、KM5 (程控板对应S8、S9继电器，以及S" + index + ")";
            Thread.Sleep(500);
            ...
            try
            {
                while (ResultData.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //状态|数据项|下限|上限|测量值
                            data = "接地测试|接地|-|" + HISETResistance + "|" + ReturnStrS[3].Replace("ohm", "Ω");
