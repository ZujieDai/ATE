...
{
    /// <summary>
    /// 欧标研测直流：切断电动汽车的电源（断开S2，测CP电压6V->9V）
    /// </summary>
    public class CCS2_RT_DC_De_energizationEVPowerSupplyTest : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        string itemFlow = "";
        public CCS2_RT_DC_De_energizationEVPowerSupplyTest(int type) { TrialType = type; }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (testWorkParam.lstIDs.Count > 0)
                    {
                        ...
                        Thread.Sleep(100);
                        ControlEquipMent.Oscilloscope.Oscilloscope_Channel_Set(testWorkParam.lstIDs, 3, true, "DC", "20", Channel3, "CP_Voltage", "50", "V", false, "10", "2.5");
                        Thread.Sleep(100);
                        ...
                        //模拟CP断线
                        SendNoticeToUIAndTxtFile("正在发送CP断线命令中...");
                        bool[] Ks = new bool[24];
                        Ks[0] = true;//DC+DC-控制
                        Ks[1] = true;//CC信号控制
                        Ks[2] = false;//CP信号控制
                        ...
                        ProcessDataTmp(Data_Tmp, "切断电动汽车的电源", "切断后CP电压(V)", "11.4", "12.6");
                        #endregion
                    }
                }
            }
