...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标研测直流：相容性评估
    /// </summary>
    public class CCS2_RT_DC_CompatibilityAssessment : BusinessBase
    {
        public CCS2_RT_DC_CompatibilityAssessment(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            ResiLoadCurrent = (int)Math.Round(MaxOutputPower * 1000.0 / BMSDemandVolt);

            //BMS需求电压(V)=500|BMS需求电流(A)=20
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (!CheckSwipingCard(testWorkParam.lstIDs, BMSDemandVolt, ResiLoadCurrent + 10))
                    {
                        return;
                    }
                    ...
                    foreach (var item in testWorkParam.lstIDs)
                    {
                        ...
                        while (timeout-- > 0)
                        {
                            if (DCVoltage.GetValueOrDefault() >= BMSDemandVolt - 10 && DCVoltage.GetValueOrDefault() <= BMSDemandVolt + 10)
                            {
                                break;
                            }
                            System.Threading.Thread.Sleep(1000);
                    ...
                    ProcessDataTmp(dic, "检测充电中电压", "充电电压(V)", (BMSDemandVolt - 10).ToString(), (BMSDemandVolt + 10).ToString());
                    ...
                    foreach (int item in testWorkParam.lstIDs)
                    {
                        ...
                        while (timeout2-- > 0)
                        {
                            ...
                            if (DCCurrent.GetValueOrDefault() >= (ResiLoadCurrent) * 0.9 && DCCurrent.GetValueOrDefault() <= (ResiLoadCurrent) * 1.1)
                            {
                                break;
                            }
                            System.Threading.Thread.Sleep(1000);
                        }
                        dicC.Add(item, DCCurrent.GetValueOrDefault().ToString("F2"));
                    }

                    //ProcessDataTmp(dic, "检测充电中电压1", "充电电压(V)", (BMSDemandVolt - 10).ToString(), (BMSDemandVolt + 10).ToString());
                    ProcessDataTmp(dicC, "检测充电中电流", "充电电流(A)", ((ResiLoadCurrent) * 0.9).ToString(), ((ResiLoadCurrent) * 1.1).ToString());
