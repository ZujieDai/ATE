...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标研测直流：充电前绝缘检测
    /// </summary>
    public class CCS2_RT_DC_InsulationBeforeCharging : BusinessBase
    {
        public CCS2_RT_DC_InsulationBeforeCharging(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    Ks[4] = true;//PE信号控制
                    Ks[4] = true;//PE信号控制
                    ControlEquipMent.BMS.BMSSetKState_EU_DC(testWorkParam.lstIDs, BatteryVoltage_EU, Ks.ToArray(), 0, 5, "0");
                    Thread.Sleep(300);
