...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标研测直流：输出电压调节
    /// </summary>
    public class CCS2_RT_DC_OutputVoltageRegulationCVC : BusinessBase
    {
        int trlTimeOut_S = 30;
        double Error = 2;
        ...
        double BMSDemandVolt3 = 0;

        public CCS2_RT_DC_OutputVoltageRegulationCVC(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            BMSDemandCurrent = MaxAllowChargeCurrent;

            //BMS需求电流(A)=6|BMS需求电压1(V)=300|BMS需求电压2(V)=400|BMS需求电压3(V)=500
            if (strParams.Length >= 5)
            {
                BMSDemandCurrent = Convert.ToDouble(strParams[1].Split('=')[1]);
                BMSDemandVolt1 = Convert.ToDouble(strParams[2].Split('=')[1]);
                BMSDemandVolt2 = Convert.ToDouble(strParams[3].Split('=')[1]);
                BMSDemandVolt3 = Convert.ToDouble(strParams[4].Split('=')[1]);
            }
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                for (int v = 0; v < BMSVolts.Length; v++)
                {
                    ...
                    {
                        var data = AllEquipStateData.DicBMS_EU_DC_StateData[testWorkParam.lstIDs[0]].ChargingCurrent;
                        int count = 30;
                        while (count-- > 0)
                        {
                            if (data < current * 0.9 || data > current * 1.1)
                            {
                                data = AllEquipStateData.DicBMS_EU_DC_StateData[testWorkParam.lstIDs[0]].ChargingCurrent;
                                Thread.Sleep(1000);
                            }
                            else
                            {
