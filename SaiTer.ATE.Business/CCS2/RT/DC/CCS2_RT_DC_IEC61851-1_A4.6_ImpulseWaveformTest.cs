...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标研测直流：PWM波脉冲波形试验
    /// </summary>
    public class CCS2_RT_DC_ImpulseWaveformTest : BusinessBase
    {
        Dictionary<int, string> Data_Tmp = new Dictionary<int, string>();//临时测试数据
        Dictionary<int, string> dImgs = new Dictionary<int, string>();//图片存储

        public CCS2_RT_DC_ImpulseWaveformTest(int trialType) { TrialType = trialType; }
...
                if (testWorkParam.lstIDs.Count <= 0)
                ...
                try
                {
                    ...
                    ProcessDataTmp(Data_Tmp, "PWM波脉冲波形试验(状态B)", "CP电压(V)", "8.37", "9.59", dImgs);
                    ...
                    ProcessDataTmp(dd, "PWM波脉冲波形试验(状态B)", "CP上升时间(us)", "0", "10");
                    ...
                    ProcessDataTmp(dd, "PWM波脉冲波形试验(状态B)", "CP下降时间(us)", "0", "13");
                    ...
                    dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
                    Data_Tmp = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "TOP", 3);//高值
                    ProcessDataTmp(Data_Tmp, "PWM波脉冲波形试验(状态C)", "CP正电压(V)", "5.47", "6.53", dImgs);
                    ...
                    ProcessDataTmp(dd, "PWM波脉冲波形试验(状态C)", "CP上升时间(us)", "0", "7");
                    ...
                    ProcessDataTmp(dd, "PWM波脉冲波形试验(状态C)", "CP下降时间(us)", "0", "13");

                    //恢复滤波功能
                    lstConditionState[14] = false;
                    ControlEquipMent.ControlBoard.ControlResistanceSetRelay(lstConditionState);
