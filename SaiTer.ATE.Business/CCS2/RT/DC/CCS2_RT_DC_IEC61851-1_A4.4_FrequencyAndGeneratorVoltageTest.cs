...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标直流研测：CP频率和电压测试
    /// </summary>
    public class CCS2_RT_DC_FrequencyAndGeneratorVoltageTest : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public CCS2_RT_DC_FrequencyAndGeneratorVoltageTest(int trialType) { TrialType = trialType; }
...
                if (testWorkParam.lstIDs.Count <= 0)
                ...
                try
                {
                    ...
                    ProcessDataTmp(Data_Tmp, "状态A", "CP电压(V)", "11.4", "12.6", dImgs);
                    ...
                    ProcessDataTmp(Data_Tmp, "状态B", "CP正电压(V)", "8.37", "9.59", dImgs);
                    ...
                    dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
                    Data_Tmp = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "TOP", 3);//高值
                    ProcessDataTmp(Data_Tmp, "状态C", "CP正电压(V)", "5.47", "6.53", dImgs);

                    //ControlEquipMent.Oscilloscope.Oscilloscope_Channel_Set(testWorkParam.lstIDs, 3, true, "DC", "20", Channel3, "CP_Voltage", "50", "V", false, "2", "4");
                    //Thread.Sleep(500);
                    Data_Tmp = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "BASE", 3);//低值
                    ProcessDataTmp(Data_Tmp, "状态C", "CP负电压(V)", "-12.6", "-11.4");

                    Data_Tmp = ControlEquipMent.Oscilloscope.Oscilloscope_ReadMeasure(testWorkParam.lstIDs, "FREQ", 3);//频率
                    ProcessDataTmp(Data_Tmp, "状态C", "CP频率(Hz)", "995", "1005");
