...
namespace SaiTer.ATE.Business
{
    ...
    /// **********************************需要改成互操开关做充电前短路***************************
    /// </summary>
    public class CCS2_RT_DC_ShortCircuitBeforeCharging : BusinessBase
    {
        ...
        public CCS2_RT_DC_ShortCircuitBeforeCharging(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, DemandVoltage, DemandCurrent, true, BatteryVoltage_EU);
                    ...
                    ProcessDataTmp(dic, "充电前短路", "充电电压(V)", "0", "20");
        ...
        public override void ProcessData()
        {
            try
            {
                foreach (var item in testWorkParam.lstIDs)
                {
                    ...
                    int state = ChangeBMSChargeStatus_EU_DC(AllEquipStateData.DicBMS_EU_DC_StateData[item].SystemState);
                    if (state >= 20)
                    {
                    ...
                    //}
                    //界面展示的数据项格式   
                    LstTrialData[i].ExtentData = "充电前短路|是否停止充电过程|-|-|" + State;
