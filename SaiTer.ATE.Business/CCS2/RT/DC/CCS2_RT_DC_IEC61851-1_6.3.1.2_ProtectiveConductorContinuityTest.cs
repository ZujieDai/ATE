...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标研测交流：保护导体的连续导通检查
    /// </summary>
    public class CCS2_RT_DC_ProtectiveConductorContinuityTest : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public CCS2_RT_DC_ProtectiveConductorContinuityTest(int type) { TrialType = type; }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (testWorkParam.lstIDs.Count > 0)
                    {
                        ...
                        ControlEquipMent.Oscilloscope.Oscilloscope_Channel_Set(testWorkParam.lstIDs, 1, true, "DC", "20", Channel1, "Output_Voltage", "50", "V", false, "250", "-3.5");
                        Thread.Sleep(100);
                        ...
                        //模拟PE断针
                        SendNoticeToUIAndTxtFile("正在发送PE断针命令中...");
                        bool[] Ks = new bool[24];
                        Ks[0] = true;//DC+DC-控制
                        Ks[1] = false;//CC信号控制
                        Ks[2] = true;//CP信号控制
                        ...
                        ProcessDataTmp(dd, "PE断针", "电源切断时间(ms)", "0", "100", dImgs);
                        ...
                        ProcessDataTmp(Data_Tmp, "PE断针", "断线后输出电压(V)", "0", "60");
                        #endregion
                        ...
                        #region CP短路
                        //设置触发
                        if (ControlEquipMent.Oscilloscope.DitEquipMentBase.Values.FirstOrDefault(e => e.EquipMentClassName.Equals("SDSOscilloscope")) != null)
                            ControlEquipMent.Oscilloscope.Oscilloscope_Trigger(testWorkParam.lstIDs, 1, "RISE", "DC", "EDGE", "10", 3, trigMaxVol, "Single");
                        else
                        ...
                        Thread.Sleep(3500);
                        SendNoticeToUIAndTxtFile("模拟导引CP短路");
                        ...
                        dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
                        ProcessDataTmp(dd, "CP短路", "电源切断时间(ms)", "0", "3000", dImgs);
                        ...
                        ProcessDataTmp(Data_Tmp, "CP短路", "断线后输出电压(V)", "0", "20");
                        #endregion
                    }
                }
            }
