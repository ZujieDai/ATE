...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标研测交流：保护导体的连续导通检查
    /// </summary>
    public class CCS2_RT_AC_ProtectiveConductorContinuityTest : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        string itemFlow = "";
        public CCS2_RT_AC_ProtectiveConductorContinuityTest(int type) { TrialType = type; }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (testWorkParam.lstIDs.Count > 0)
                    {
                        ...
                        dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
                        ProcessDataTmp(dd, "CP接地", "电源切断时间(ms)", "0", "3000", dImgs);
                        ...
                        ProcessDataTmp(Data_Tmp, "CP接地", "断线后输出电压(V)", "0", "20");
                        #endregion
                    }
                }
            }
