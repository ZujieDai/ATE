...
namespace SaiTer.ATE.Business
{
    ...
    /// 欧标研测交流：充电模式3
    /// </summary>
    public class CCS2_RT_AC_ChargingMode3Test : BusinessBase
    {
        string itemFlow = "";
        public CCS2_RT_AC_ChargingMode3Test(int type) { TrialType = type; }
        ...
        public void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    itemFlow = "控制先导功能";
                    string info = $"将电动汽车永久连接到交流电网的交流电动汽车供电设备，具有从交流电动汽车供电设备延伸到电动汽车的控制先导功能。\r\n请确认是否合格(勾选上为合格)";
                    CountDownTimeInfo(info, CheckTime, 2);
                    ProcessData();

                    itemFlow = "保护性接地导线";
                    info = $"供电设备应为电动汽车插座和/或车辆连接器提供一个保护性接地导线。\r\n请确认是否合格(勾选上为合格)";
                    CountDownTimeInfo(info, CheckTime, 2);
                    ProcessData();
                }
            }
