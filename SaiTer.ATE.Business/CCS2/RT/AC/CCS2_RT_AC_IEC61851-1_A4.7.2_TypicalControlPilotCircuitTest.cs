...
{
    /// <summary>
    /// 欧标研测交流：使用典型控制导引电路的时序测试
    /// </summary>
    public class CCS2_RT_AC_TypicalControlPilotCircuitTest : BusinessBase
    {
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public CCS2_RT_AC_TypicalControlPilotCircuitTest(int trialType) { TrialType = trialType; }
...
                if (testWorkParam.lstIDs.Count <= 0)
        ...
        {
            //1.1 --> 3.1 --> 4 --> 7 --> 8.1.--> 4 --> 6 --> 7 --> 8.1 --> 2.1.--> 9.3
            //断开CP,S2
            ...
            ProcessDataTmp(dd, sState, "A1->B1(1.1)时间(ms)", "-", "-", dImgs);
            #endregion
            ...
            CPPWMDownTime(testWorkParam.lstIDs, 3, 8.2 + diff, 1, 2);
            CPPWMDownTime(testWorkParam.lstIDs, 3, -9.2 + diff, 2, 2);
            //读取卡点时间
            ControlEquipMent.Oscilloscope.Oscilloscope_ReadCursors(testWorkParam.lstIDs, 1, ref OscTime_Tmp);
            ...
            dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
            ProcessDataTmp(dd, sState, "B1->B2(3.1)变化时间(ms)", "-", "-", dImgs);
            #endregion
            ...
            dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
            ProcessDataTmp(dd, sState, "B2->C2(4)变化时间(ms)", "-", "-", dImgs);
            #endregion
            ...
            dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
            ProcessDataTmp(dd, sState, "C2->B2(7),(8.1)变化时间(ms)", "-", "-", dImgs);
            #endregion
            ...
            dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
            ProcessDataTmp(dd, sState, "B2->A2(2.1)变化时间(ms)", "-", "-", dImgs);
            #endregion
            ...
            CPPWMDownTime(testWorkParam.lstIDs, 3, -11.5, 1, 1);
            CPPWMDownTime(testWorkParam.lstIDs, 3, 11.4, 2, 1);
            //读取卡点时间
            ControlEquipMent.Oscilloscope.Oscilloscope_ReadCursors(testWorkParam.lstIDs, 1, ref OscTime_Tmp);
            ...
            dImgs = ControlEquipMent.Oscilloscope.OscilloscopeSaveScreen(testWorkParam.lstIDs);
            ProcessDataTmp(dd, sState, "A2->A1(9.3)变化时间(ms)", "-", "-", dImgs);
            #endregion
        }
