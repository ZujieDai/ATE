...
namespace SaiTer.ATE.Business
{
    ...
    public class CCS2_PT_DC_ProtectiveGroundingContinuity : BusinessBase
    {
        public CCS2_PT_DC_ProtectiveGroundingContinuity(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    SendNoticeToUIAndTxtFile("正在发送PE断线指令");
                    ...
                    Ks[2] = true;//CP信号控制
                    Ks[4] = false;//PE信号控制
                    ControlEquipMent.BMS.BMSSetKState_EU_DC(lstIDs, BMSMeasureVoltage, Ks.ToArray(), 0, 5, "0");
