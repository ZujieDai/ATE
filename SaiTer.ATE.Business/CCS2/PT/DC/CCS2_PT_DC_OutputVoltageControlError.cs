...
namespace SaiTer.ATE.Business
{
    ...
    /// 输出电压控制误差
    /// </summary>
    public class CCS2_PT_DC_OutputVoltageControlError : BusinessBase
    {
        int trlTimeOut_S = 30;
        ...
        public CCS2_PT_DC_OutputVoltageControlError(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                if (!CheckSwipingCard(testWorkParam.lstIDs, OutputVoltage1, MaxAllowChargeCurrent))
                {
                    return;
                }
                ...
                SendNoticeToUIAndTxtFile(string.Format("设置带载电压" + OutputVoltage1 + "V,带载电流" + "{0}A，等待负载稳定", current.ToString("F1")));
                SetLoadPara(testWorkParam.lstIDs, OutputVoltage1 - 20, current, OutputVoltage1, current);
                Thread.Sleep(500);
