...
namespace SaiTer.ATE.Business
{
    ...
    /// 自检阶段测试
    /// </summary>
    public class CCS2_PT_DC_SelfInspectionStage : BusinessBase
    {
        public CCS2_PT_DC_SelfInspectionStage(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (!CheckSwipingCard(testWorkParam.lstIDs, 500, 250))
                    {
                        return;
                    }
                    ...
                    foreach (var item in testWorkParam.lstIDs)
                    {
                        ...
                        while (time-- > 0)
                        {
                            if (DCVoltage >= 480 && DCVoltage <= 520)
                                break;
                    ...
                    ProcessDataTmp(dic, "电池过压", "过压值(V)", "-", "-");

                    ControlEquipMent.BMS.SetParameter(lstIDs, 500, 250, true, 500);
                    Thread.Sleep(200);
                    ControlEquipMent.BMS.BMS_ON(lstIDs);
                    SystemEvent.SendWaitSwipingCard(lstIDs, 500, LstChargerInfo[0].ChargerType, 0);
                    Thread.Sleep(2000);

                    ProcessDataChargeState();
