...
namespace SaiTer.ATE.Business
{
    ...
    /// 输出电流测量误差
    /// </summary>
    public class CCS2_PT_DC_CurrentMeasureError : BusinessBase
    {
        public CCS2_PT_DC_CurrentMeasureError(int type)
        {
            TrialType = type;
        }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (IsRLoad(DemandVoltage3, DemandCurrent1))
                    {
                        ...
                        var dicC = GetPowerAnalyzerCurrent(testWorkParam.lstIDs, CheckCurrent1, CheckCurrent2);
                    ...
                    if (IsRLoad(DemandVoltage2, DemandCurrent5))
                    {
                        ...
                        ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, DemandVoltage2, DemandCurrent5 + 5, true, 390);
                    ...
                    if (IsRLoad(DemandVoltage1, DemandCurrent9))
                    {
                        ...
                        SetLoadDCON(testWorkParam.lstIDs);
                        //CountDownTimeInfo("等待带载稳定", 20, 0);
                        WaitDCCurrent_EU_DC(testWorkParam.lstIDs, DemandCurrent9);
                        Thread.Sleep(3 * 1000);
