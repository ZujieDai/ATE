...
namespace SaiTer.ATE.Business
{
    ...
    /// 老化试验
    /// </summary>
    public class TPK_AgingTest : BusinessBase
    {
        public TPK_AgingTest(int trialType) { TrialType = trialType; }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                ProcessDataTmp(dicAgingVolt, "老化1秒", "实际电压(V)", ((1.0 - VoltError / 100) * AgingVolt).ToString(), ((1.0 + VoltError / 100) * AgingVolt).ToString());
                ProcessDataTmp(dicAgingCurr, "老化1秒", "实际电流(V)", ((1.0 - CurrError / 100) * AgingCurr).ToString(), ((1.0 + CurrError / 100) * AgingCurr).ToString());
                ...
                while (sw.ElapsedMilliseconds / 1000 <= AgingTime * 60)
                {
                    ...
                    ProcessDataTmp(dicAgingVolt, string.Format("老化{0}秒", time), string.Format("实际电压(V)", time), ((1.0 - VoltError / 100) * AgingVolt).ToString(), ((1.0 + VoltError / 100) * AgingVolt).ToString());
                    ProcessDataTmp(dicAgingCurr, string.Format("老化{0}秒", time), string.Format("实际电流(A)", time), ((1.0 - CurrError / 100) * AgingCurr).ToString(), ((1.0 + CurrError / 100) * AgingCurr).ToString());
                }
