...
namespace SaiTer.ATE.Business
{
    ...
    public class TPK_CC1DisConnection : BusinessBase
    {
        int CC1KSIndex = -1;    //CC1程控板开关(索引号)
        int CC1CHNum = -1;      //CC1示波器通道号
...
        Dictionary<int, double[]> OscTime_Tmp = new Dictionary<int, double[]>();//示波器光标卡点时间
        ...
        public TPK_CC1DisConnection(int trialType) { TrialType = trialType; }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                ControlEquipMent.Oscilloscope.Oscilloscope_Measure_Initialize(testWorkParam.lstIDs);//清除所有测量项     弹出报错    CC1 检测 需要示波器吗？
                ControlEquipMent.Oscilloscope.Oscilloscope_Channel_Set(testWorkParam.lstIDs, 1, true, "DC", "20", Channel1, "Output_Voltage_DC", "1", "V", false, "300", "-3");
                ...
                if (CC1CHNum > -1)
                {
                    ControlEquipMent.Oscilloscope.Oscilloscope_Channel_Set(testWorkParam.lstIDs, CC1CHNum, true, "DC", "20", "1", "CC1", "1", "A", false, "2", "0");
                    Thread.Sleep(waitTime);
                }
                ...
                if (CC1CHNum > -1)
                {
                    ControlEquipMent.Oscilloscope.Oscilloscope_Trigger(testWorkParam.lstIDs, 0, "RISE", "DC", "EDGE", "100", CC1CHNum, "5.2", "Single");
                }
                //Thread.Sleep(waitTime);
                ...
                //读取分析数据
                Thread.Sleep(1500);
                //ACDownTime(testWorkParam.lstIDs, 2, 20 - 5, 1);
                //ACDownTime(testWorkParam.lstIDs, 2, 3, 2);//AC回路动作时间
                if (CC1CHNum > -1)
                {
                    ACUpTime(testWorkParam.lstIDs, CC1CHNum, 5.2, 1);
