...
{
    public class TPK_LED_GB_DC : BusinessBase
    {


        public TPK_LED_GB_DC(int trialType)
        {
            TrialType = trialType;
        }
            ...
            public void StartItemFlow()
            {
                ...
                if (testWorkParam.lstIDs.Count == 0)
                 ...
                 string info = "请检查（电源指示灯）是否正常点亮。\r\n请确认是否合格 注：勾选上为合格";
                CountDownTimeInfo(info, 30, 2);
                ProcessDataResults(testWorkParam.lstIDs, "-", "-", DicManualVerifyResult, "电源指示灯检查");

                // CountDownTimeInfo("请将充电桩辅源电压设置到12V ", 60, 2);      提示刷卡充电  不需要带载//  ControlEquipMent.BMS.BMS_ON(testWorkParam.lstIDs);
                // SetCPReresh();
                //ControlEquipMent.BMS.SetParameter(testWorkParam.lstIDs, ChargingVolt, ChargingCurr + 10, true, ChargingVolt);  //                                                                                            
                ...
                info = "充电中，请检查（运行指示灯）是否正常点亮。\r\n请确认是否合格 注：勾选上为合格";
                CountDownTimeInfo(info, 30, 2);
                ProcessDataResults(testWorkParam.lstIDs, "-", "-", DicManualVerifyResult, "运行指示灯检查");
                Thread.Sleep(2000);
                CountDownTimeInfo("充电中，请按急停模拟故障，检查（故障指示灯）是否正常点亮。\r\n请确认是否合格 注：勾选上为合格", 60, 2);
                ProcessDataResults(testWorkParam.lstIDs, "-", "-", DicManualVerifyResult, "故障指示灯检查");
                Thread.Sleep(1000);
