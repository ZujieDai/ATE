...
namespace SaiTer.ATE.Business
{
    ...
    /// </summary>
    public class CZ_TB_InputVoltageError : BusinessBase
    {
        Dictionary<int, string> Data_Tmp = new Dictionary<int, string>();//临时测试数据

        double InputVoltage = 220;  //异常电压
        double NormalVoltage = 220; //额定电压
        double RecoverVoltage = 220; //恢复电压
        double WaitTime = 20;

        double VoltageRate = 1.3;//电压倍率  输入电压最高不能超过额定电压的倍数
        public CZ_TB_InputVoltageError(int type)
        {
            TrialType = type;
        }
        ...
        public override void InitializeParams()
        {
            ...
            RecoverVoltage = Convert.ToDouble(strParams[1].Split('=')[1]);
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                if (testWorkParam.lstIDs.Count > 0)
                {
                    ...
                    SendNoticeToUIAndTxtFile("恢复正常电压：" + RecoverVoltage + "V，等待交流源输出稳定。");
                    //ControlEquipMent.ACSource.ACSource_SetVolt(lstIDs, NormalVoltage);
                    SetACSource(testWorkParam.lstIDs, RecoverVoltage);
                    //ControlEquipMent.ACSource.ACSource_ON(testWorkParam.lstIDs);
