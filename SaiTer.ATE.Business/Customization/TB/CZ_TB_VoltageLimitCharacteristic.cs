...
namespace SaiTer.ATE.Business
{
    ...
    /// 限压特性
    /// </summary>
    public class CZ_TB_VoltageLimitCharacteristic : BusinessBase
    {
        int trlTimeOut_S = 30;
        double OutputVoltage = 0;

        double ResiLoadCurrent = 0;
        public CZ_TB_VoltageLimitCharacteristic(int type)
        {
            TrialType = type;
        }
        ...
        public void StartItemFlow()
        {
            ...
            while (true)
            {
                ...
                SendNoticeToUIAndTxtFile(string.Format("设置带载电压" + (OutputVoltage - 10) + "V,带载电流" + "{0}A，等待负载稳定", ResiLoadCurrent.ToString("F1")));
                ...
                double minData = ResiLoadCurrent - currentError;
                double maxData = ResiLoadCurrent + currentError;
                for (int i = 0; i < testWorkParam.lstIDs.Count; i++)
                {
                ...
                SetLoadPara(testWorkParam.lstIDs, OutputVoltage - 10, ResiLoadCurrent - 3, OutputVoltage + 5, ResiLoadCurrent);
                SetLoadDCON(testWorkParam.lstIDs);
                //WaitDCCurrent_EU_DC(testWorkParam.lstIDs, ResiLoadCurrent);
                Thread.Sleep(3000);
