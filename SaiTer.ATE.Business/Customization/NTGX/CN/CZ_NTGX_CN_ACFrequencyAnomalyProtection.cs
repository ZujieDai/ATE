...
namespace SaiTer.ATE.Business
{
    ...
    /// 频率异常保护检测（过欠频测试）
    /// </summary>
    public class CZ_NTGX_CN_ACFrequencyAnomalyProtection : BusinessBase
    {
        public CZ_NTGX_CN_ACFrequencyAnomalyProtection(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    //模拟电网侧频率
                    SendNoticeToUIAndTxtFile("模拟电网侧频率【" + FreqUp.ToString() + " Hz】...");
                    ControlEquipMent.ACSource.ACSource_SetFreq(testWorkParam.lstIDs, FreqUp);
                    ControlEquipMent.ACSource.ACSource_ON(testWorkParam.lstIDs);
                    Thread.Sleep(3000);//启动前设置频率会恢复为50，这里再发一次
                    ControlEquipMent.ACSource.ACSource_SetFreq(testWorkParam.lstIDs, FreqDown);
                    ...
                    ProcessDataTmp(dic, "过频保护" + FreqUp, "放电电压(V)", (0).ToString(), (20).ToString());//需要停止充电
                    ...
                    //模拟电网侧频率
                    SendNoticeToUIAndTxtFile("模拟电网侧频率【" + FreqDown.ToString() + " Hz】...");
                    ControlEquipMent.ACSource.ACSource_SetFreq(testWorkParam.lstIDs, FreqDown);
                    ControlEquipMent.ACSource.ACSource_ON(testWorkParam.lstIDs);
                    Thread.Sleep(3000);//启动前设置频率会恢复为50，这里再发一次
                    ControlEquipMent.ACSource.ACSource_SetFreq(testWorkParam.lstIDs, FreqDown);
                    ...
                    ProcessDataTmp(dic, "欠频保护" + FreqDown, "放电电压(V)", (0).ToString(), (20).ToString());//需要停止充电
