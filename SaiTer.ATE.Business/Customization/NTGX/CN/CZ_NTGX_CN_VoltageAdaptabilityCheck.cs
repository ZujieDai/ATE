...
namespace SaiTer.ATE.Business
{
    ...
    /// 电压适应性检测
    /// </summary>
    public class CZ_NTGX_CN_VoltageAdaptabilityCheck : BusinessBase
    {
        public CZ_NTGX_CN_VoltageAdaptabilityCheck(int type)
        {
            TrialType = type;
        }
        //放电电压(V)=220|起始电压(V)=210|终止电压(V)=240|电压变化步进(V)=5|运行时间(s)=5
        private int TestTime = 0;//测试时间
        double BMSDemandVolt = 0;//额定电压
        double BMSDemandCurrent = 0;//额定电流
        double VoltStart = 49.55;
        double VoltEnd = 50.15;
        double Bj = 5;
        double RunTime_s = 5;
        int trlTimeOut_S = 0;
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    int iCnt = 0;
                    double dVolt = VoltStart;
                    while (true)//目前固定3分钟
                    {
                        dVolt = VoltStart + (Bj * iCnt);
                        if (dVolt > VoltEnd) dVolt = VoltEnd;

                        //调节电网模拟器频率
                        SendNoticeToUIAndTxtFile("模拟电网侧电压【" + dVolt.ToString() + " V】...");
                        ControlEquipMent.ACSource.ACSource_SetVolt(testWorkParam.lstIDs, dVolt);
                        ControlEquipMent.ACSource.ACSource_SetFreq(testWorkParam.lstIDs, 50);
                        ...
                        ProcessDataTmp(dic, "电网侧电压 " + dVolt, "充电电压(V)", (BMSDemandVolt - 30).ToString(), (BMSDemandVolt + 30).ToString());


                        iCnt++;
                        if (dVolt >= VoltEnd) break;
                    }
        ...
        public override void InitializeParams()
        {
            try
            {
                ...
                VoltStart = double.Parse(strParams[1].Split('=')[1]);
                VoltEnd = double.Parse(strParams[2].Split('=')[1]);
