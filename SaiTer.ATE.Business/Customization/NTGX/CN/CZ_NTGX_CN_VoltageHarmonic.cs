...
namespace SaiTer.ATE.Business
{
    ...
    /// 电压谐波
    /// </summary>
    public class CZ_NTGX_CN_VoltageHarmonic : BusinessBase
    {
        //直流分量计算公式
        ...
        public CZ_NTGX_CN_VoltageHarmonic(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    List<double> lst = ControlEquipMent.PowerAnalyzer.ReadVoltageHarmonicValue_50(testWorkParam.lstIDs, 1);
                    if (lst.Count > 0)
                    {
                        for (int i = 0; i < lst.Count; i++)
                        {
                            ...
                            if (i == 0)
                            {
                                ProcessDataTmp(dic, "电压谐波" + (i + 1).ToString() + "(基波)", "谐波含量(%)", "-", "-");
                            }
                            else
                            {
                                ProcessDataTmp(dic, "电压谐波" + (i + 1).ToString(), "谐波含量(%)", (0).ToString(), (ErrLimit).ToString());
                            }
                        }
                    }
                    else
                    {
                        ...
                        ProcessDataTmp(dic, "电压谐波", "谐波含量(%)", (0).ToString(), (ErrLimit).ToString());
                    }


                    double dTotal = ControlEquipMent.PowerAnalyzer.ReadVoltageHarmonic_Total(testWorkParam.lstIDs, 1);
                    ProcessDataTmp(dic, "电压谐波", "电压谐波总失真度（%）", (0).ToString(), (ErrLimit).ToString());
