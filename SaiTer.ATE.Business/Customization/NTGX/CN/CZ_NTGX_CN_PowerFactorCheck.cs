...
namespace SaiTer.ATE.Business
{
    ...
    /// 功率因数检查
    /// </summary>
    public class CZ_NTGX_CN_PowerFactorCheck : BusinessBase
    {
        public CZ_NTGX_CN_PowerFactorCheck(int type)
        {
            TrialType = type;
        }
        ...
        private void StartItemFlow()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    CountDownTimeInfo("请设定交流放电功率【" + Power_kW + " kW】，以及功率因数【0.98】", 30, 0);
                    ...
                    foreach (var item in testWorkParam.lstIDs)
                    {
                        double DCVoltage =(double) AllEquipStateData.DicPowerAnalyzer_StateData.FirstOrDefault().Value?.TotalPowerFactor;
                        DCVoltage = Math.Abs(DCVoltage);
                        dic.Add(item, DCVoltage.ToString("F2"));
                    }
                    ProcessDataTmp(dic, "放电状态", "功率因数", (0.98 - ErrLimit < 0 ? 0 : 0.98 - ErrLimit).ToString(), (0.98 + ErrLimit < 1 ? 1 : 0.98 + ErrLimit).ToString());
