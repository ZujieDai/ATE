...
namespace SaiTer.ATE.UI.UserControls.EquipOperate
{
    ...
    /// 回馈负载
    /// </summary>
    public partial class UcFeedbackLoad : UcEquipOperateBase
    {
        public UcFeedbackLoad()
        {
            InitializeComponent();

            string Customer = ConfigurationManager.AppSettings["Customer"];
            if (Customer != null && Customer.ToString().ToUpper().Equals("YKR"))
            {
                btnConnect.Visible = false;
                btnOpen.Visible = false;
                btnBMSON.Visible = true;
                btnBMSOFF.Visible = true;
            }
        }
        ...
        {
            EquipMentControl.FeedbackLoad.FeedbackLoad_BMSON(lstChargerID);
        }

        private void btnBMSOFF_Click(object sender, EventArgs e)
        {
            EquipMentControl.FeedbackLoad.FeedbackLoad_BMSOFF(lstChargerID);
        }
    }
}
