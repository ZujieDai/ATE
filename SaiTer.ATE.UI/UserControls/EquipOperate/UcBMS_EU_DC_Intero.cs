...
{
    public partial class UcBMS_EU_DC_Intero : UcEquipOperateBase
    {
        bool isRead = false;
        public UcBMS_EU_DC_Intero()
        {
            InitializeComponent();
        }

        private void UcBMS_EU_DC_Intero_Load(object sender, EventArgs e)
        {
            GetChargerID();
        ...
        {
            EquipMentControl.BMS.BMS_SetResistance(lstChargerID, Convert.ToUInt16(txtR2.Text), Convert.ToUInt16(txtR3.Text), new string[] { "emtBMS_EU_DC" });
        }

        private void btnRead_R_Click(object sender, EventArgs e)
        {
            ushort r2 = 0, r3 = 0;
            EquipMentControl.BMS.BMS_GetResistance(lstChargerID, ref r2, ref r3);
        ...
        {
            isRead = true;
            var dic = EquipMentControl.BMS.BMSGetKState_EU_DC(lstChargerID, out double voltage, new string[] { "emtBMS_EU_DC" });
            List<int> lstKStage = dic[lstChargerID[0]];
            if (lstKStage == null || lstKStage.Count < 1)
                return;
        ...
        private void SetKState()
        {
            ...
            EquipMentControl.BMS.BMSSetKState_EU_DC(lstChargerID, voltage, list.ToArray(), DCPlus, DCMinus, "", new string[] { "emtBMS_EU_DC" });
        }

        private void btnVersion_Click(object sender, EventArgs e)
        {
            FrmBMSVersion frm = FrmBMSVersion.GetInstance(lstChargerID, new string[] { "emtBMS_EU_DC" });
            frm.Show();
        }
        ...
        private void btn_SetEA_UA_Click(object sender, EventArgs e)
        {
            ...
            {
                EquipMentControl.BMS.BMSSetHCAC(lstChargerID, EmChargerType.Charger_USA_DC, new string[] { "emtBMS_EU_DC" });
                btn_SetEA_UA.Text = "切换欧标";
            }
            else
            {
                EquipMentControl.BMS.BMSSetHCAC(lstChargerID, EmChargerType.Charger_EUR_DC, new string[] { "emtBMS_EU_DC" });
