...
namespace SaiTer.ATE.EquipMent
{
    ...
    public class emtBMS_EU_DC : EquipMentBase
    {
        ...
        /// <param name="ReadyVolt">预充电压</param>
        public override void BMSSetParameter(Double bmsVolt, Double bmsCurrent, bool type, double ReadyVolt, bool canCharge = true, InsulationState insulationState = InsulationState.正常)
        {
            //bmsCurrent = bmsCurrent >= 250 ? 250 : bmsCurrent;
            //byte[] WriteBuffer = procotol.BMSSetPara(bmsVolt, bmsCurrent, type, measureVolt);
            byte[] WriteBuffer = procotol.BMSSetPara2_EU_DC(100, 85, bmsVolt, bmsCurrent, ReadyVolt);
            SendData(WriteBuffer);
        }
        ...
        public override void ReadBMS_StateData()
        {
            ...
            {
                AllEquipStateData.DicBMS_EU_DC_StateData.Add(ChargerID, bmsStateData);
            }
            while (true)
            {
                try
                        ...
                        if (EquipMentPort != null)
                        {
                            for (int i = 0; i < ReConnNum; i++)
                            {
                                ...
                                if (RevMsgData != null)
                                {
                                    ...
                                    {
                                        int index = RevMsgData.ToList().IndexOf(0x7E);
                                        if (index >= 0 && index + 64 <= RevMsgData.Length)
                                        {
                                            byte[] RevMsgData1 = RevMsgData.Skip(index).Take(64).ToArray();
                                    ...
                                    if (!CheckOut.CheckCrc_EU(RevMsgData))
                                    {
                                        bmsStateData = new BMS_EU_DC_StateData();
                                    ...
                                    RevMsgData = CheckOut.Array_CharToHex(RevMsgData);
                                    bmsStateData = procotol.GetBMS_EU_DC_StateData(RevMsgData, this.ChargerID);
                                    SystemEvent.SendMonitorMessage(bmsStateData);
