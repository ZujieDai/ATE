...
namespace SaiTer.ATE.EquipMent
{
    ...
    /// 设备-KeySight3000示波器
    /// </summary>
    public class emtKSOscilloscope_3000X : EquipMentBase
    {
        ...
        ///  25=周期均方根CRMS
        ///  </summary>
        const string CS_MEASURE_TYPE = "VAMPlitude|VAVerage|FREQuency|DUTYcycle|BWIDth|DELay|DT|DUTYcycle|PPULses|" +
            "FALLtime|FREQuency|VTOP|VBASe|VMAX|VMIN|OVERshoot|NPULses|PERiod|PPULses|OVERshoot|" +
            "VAMPlitude|PWIdth|RISetime|VRMS|SDEViation|DUTYcycle";
        ...
        ///  4=多单次 NSingle
        /// </summary>
        const string CS_TRIGGER_MODE = "AUTO|NORMal|NORMal|NORMal|NORMal";
        ...
        ///  2=欠幅 Runt
        /// </summary>
        const string CS_TRIGGER_TYPE = "EDGE|EDGE|RUNT";
        ...
        ///  2=交替 ALTernate
        /// </summary>
        const string CS_TRIGGER_SLOPE = "POSitive|NEGative|EITHer";
        ...
        int rlngErrNum = 0;
        string rstrErrDescr = "";
        public emtKSOscilloscope_3000X(int type)
        {
            this.AutoReadData = true;
            this.EquipMentName = "KeySight" + " " + LanguageManager.GetByKey("示波器");
            SetImagePath();
            ...
            lock (SynLock)
            {
                try
                {
                    ...
                    if (isOpen)
                    {
                        ...
                        string[] stbBanswidths = { "200E6", "20E6", "200E6" }; // OFF是200M全带宽，ON是20M
                        string[] stbCouplings = { "AC", "DC", "DC" }; // GND本设备没有，默认DC
                        ...
                        List<string> listCommands = new List<string>()
                        {
                            ...
                            $":CHANnel{viChannel}:BANDwidth {stbBanswidths[viBandWidth]}", // 设置带宽
            ...
            lock (SynLock)
            {
                try
                {
                    ...
                    if (Count < 6)
                    {
                        ...
                        WriteString($"MEASure:{tbMeasureTypes[viMeasureType]} CHANnel{channel}");
                    }
                    else
                    {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(strCurMeasureType))
                    {
                        ...
                        if (bRet && (!strTmp.Contains("*")))
                        {
                            try
                            {
                                value = decimal.Parse(strTmp, System.Globalization.NumberStyles.Float).ToString("F3");
                            }
                            catch
                            {
                                value = "-10000";
            ...
            lock (SynLock)
            {
                try
                {
                    ...
                    List<string> listCommands = new List<string>()
                    {
                        ...
                        string.Format(":TIMebase:POSition {0}", (vfDelay/1000).ToString("e")),
                    };
                    WriteMultipleString(listCommands);
        ...
        public override void Oscilloscope_Trigger(int type_first, string type_second, string coupling, string timeout_type, string timeout, int channel, string triggerLevel, string triggerType)
        {
            lock (SynLock)
            {
                try
                {
                    ...
                    float vfTimeoutOrLower, vfLevelOrUpper;
                    ...
                    if (timeout != "")
                    {
                        vfTimeoutOrLower = (float)Convert.ToDouble(timeout);
                    }
                    vfLevelOrUpper = 10;
                    if (triggerLevel != "")
                    {
                        vfTimeoutOrLower = (float)Convert.ToDouble(triggerLevel.Replace("V", "").Replace("m", ""));
                    }
                    ...
                    if (viMainType == 2)
                    {
                        listCommands.Add(string.Format("TRIGger:{0}:TIME {1}", strTrigType, vfTimeoutOrLower));
                    }
        ...
        public override void OscilloscopeCursorData(int channnel, ref double[] rtdBuffer, int RecondLengZero = 10000 * 20)
        {
            lock (SynLock)
            {
                try
                {
                    ...
                    {
                        ":STOP",
                        $":DIGitize CHANnel{channnel}",
                        $":WAVeform:SOURce CHANnel{channnel}",
                    ...
                    string[] _WaveDataTbl = btWaveDataTemp.Split(',');
        ...
        public override void OscilloscopeSaveScreen(ref string path)
        {
            lock (SynLock)
            {
                try
                {
                    ...
                    byte[] ImageData = new byte[20 * 1024 * 1024];
                    if (Read(ImageData.Length, ref ImageData))
                    {
                        //保存图片到本地文件夹
        ...
        /// <returns></returns>
        public override void Oscilloscope_CursorPosition_X_Single(int channel, double vfPositionValue, bool isleft)
        {
            lock (SynLock)
            {
                try
                {
                    ...
                    double xCurrsorPos = (Scale * CI_CURSSOR_HNUM);
                    xCurrsorPos = xCurrsorPos * ((vfPositionValue / 100) - 0.5) + Delay;

                    List<string> listCommands = new List<string>()
                    {
                        $":MARKer:X1Y1source CHANnel{channel}",
                        $":MARKer:X{viLeftOrRight}Position {xCurrsorPos}",
                    };
