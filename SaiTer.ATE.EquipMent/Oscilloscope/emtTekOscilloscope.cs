...
namespace SaiTer.ATE.EquipMent
{
    ...
    /// 设备-泰克示波器
    /// </summary>
    public class emtTekOscilloscope : EquipMentBase
    {
        ...
        int sleeptime = 100;
        public emtTekOscilloscope(int type)
        {
            this.AutoReadData = true;
            this.EquipMentName = "TEK" + " " + LanguageManager.GetByKey("示波器");
            SetImagePath();
        ...
        public override void Oscilloscope_Trigger(int type_first, string type_second, string coupling, string timeout_type, string timeout, int channel, string triggerLevel, string triggerType)
        {
            lock (SynLock)
            {
                try
                {
                    ...
                    if (type_first == 1)
                    {
                        type_first = 0;//泰克示波器不支持超时触发，改为边沿
                    }
                    if (type_first == 0)
                    {
                    ...
                    if (type_first == 2)
                    {
                        EquipMentPort.VisaNS.Write("TRIGger:A:TYPe PULSEWidth");//设置触发类型为欠幅
                        System.Threading.Thread.Sleep(sleeptime);
        ...
        public override void Oscilloscope_TriggerTypeSet(string TriggerType)
        {
            lock (SynLock)
            {
                try
                {
                    ...
                    {
                        EquipMentPort.VisaNS.Write("ACQUIRE:STOPAFTER RUNSTOP");
                        System.Threading.Thread.Sleep(sleeptime);
                        EquipMentPort.VisaNS.Write("TRIGger:A:MODe " + TriggerType);//设置触发类型
                        System.Threading.Thread.Sleep(sleeptime);
                        EquipMentPort.VisaNS.Write("ACQuire:STATE ON");//滚动打开
                        System.Threading.Thread.Sleep(sleeptime);
                    }
        ...
        public override void Oscilloscope_ReadRun(ref bool isRun)
        {
            lock (SynLock)
            {
                ...
                {
                    this.AutoReadData = false;
                    EquipMentPort.VisaNS.Write("ACQuire:STATE?");//读取触发状态
                    System.Threading.Thread.Sleep(sleeptime);
                    string tString = EquipMentPort.VisaNS.ReadString().Trim(new char[] { '\n', '\r' });//回读测量项
                                                                                                       //string[] tStringS = tString.Split(',');
                                                                                                       //if (tStringS.Length >= 1)
        ...
        public override void Oscilloscope_CursorPosition_X(int channel, double value1, double value2)
        {
            lock (SynLock)
            {
                try
                {
                    ...
                    double temp1 = value1 * 16 - 8;// ratio * 15 - 7.5+0.05;
                    if (!string.IsNullOrEmpty(Customer) && (Customer.Equals("HYQCP") || Customer.Equals("WR")))
                    {
                        temp1 = value1 * 10 - 5;
                    }
                    ...
                    System.Threading.Thread.Sleep(sleeptime);
                    double temp2 = value1 * 16 - 8;// ratio * 15 - 7.5+0.05;
                    if (!string.IsNullOrEmpty(Customer) && (Customer.Equals("HYQCP") || Customer.Equals("WR")))
                    {
                        temp2 = value1 * 10 - 5;
                    }
        ...
        public override void ReadTriggerState(ref bool isTrigger)
        {
            ...
            lock (SynLock)
            {
                try
                {
                    ...
                    if (TriggerType.ToUpper().Contains("TRIGGER") || TriggerType.ToUpper().Contains("SAVE") || TriggerType.ToUpper().Contains("TRIG") || TriggerType.ToUpper().Contains("SAV"))
                    {
                        isTrigger = true;
                    }
                    else
