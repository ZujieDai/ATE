...
{
    public class OscillographControl : OscillographBase
    {


        private string[] ClassNames = new string[] { "emtOscillographInstrument" };
        ...
        public override void OscillographIDefalut()
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.OscillographIDefalut();
        ...
        public override void OscillographIDefalut_Show()
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                item.Value.OscillographIDefalut_Show();
            }
        }
        /// <summary>
        /// 录波仪通道设置开关
        ...
        /// <returns>返回为try catch 的错误，没有则返回为空</returns>
        public override void Oscillograph_Channel_Open(List<int> lstIDs, int channel, bool isOpen)
        {
            //设置程控板切换通道
            if (isOpen)
                SwitchChannelByControlBoard(lstIDs, "Oscillograph_Channel_" + channel, "");

            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {

                item.Value.Oscillograph_Channel_Open(channel, isOpen);
        ...
        /// <returns>返回为try catch 的错误，没有则返回为空</returns>
        public override void Oscillograph_Channel_Set(List<int> lstIDs, int channel, int TRACE, bool isOpen, string coupling, string tapewidth, string SRATE, string probe, string tag, string unit, bool isOpen_A, string gear, string position, int filteringtype, int digitfilteringtype, int digittapetype, string digittypewidth, string Color, bool SGR1, bool SGR2, bool SGR3, bool SGR4)
        {
            //设置程控板切换通道
            if (isOpen)
                SwitchChannelByControlBoard(lstIDs, "Oscillograph_Channel_" + channel, tag);

            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                item.Value.Oscillograph_Channel_Set(channel, TRACE, isOpen, coupling, tapewidth, SRATE, probe, tag, unit, isOpen_A, gear, position, filteringtype, digitfilteringtype, digittapetype, digittypewidth, Color, SGR1, SGR2, SGR3, SGR4);
        ...
        /// <param name="digittypewidth">带通滤波通带宽度 ？默认200Hz</param>
        /// <returns>返回为try catch 的错误，没有则返回为空</returns>
        public override void Oscillograph_Channel_SetFiltering(List<int> lstIDs, int channel, string tapewidth, int filteringtype, int digitfilteringtype, int digittapetype, string digittypewidth)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                item.Value.Oscillograph_Channel_SetFiltering(channel, tapewidth,  filteringtype, digitfilteringtype, digittapetype, digittypewidth);
        ...
        public override void Oscillograph_Channel_SetGear(int channel, string gear, string position)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_Channel_SetGear(channel, gear, position);
        ...
        public override void Oscillograph_Channel_SetGear_CAN(int channel, int schannel, string UpLimit, string DownLimit)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_Channel_SetGear_CAN(channel, schannel, UpLimit, DownLimit);
        ...
        public override void Oscillograph_Filtering(int channel, string coupling, string tapewidth, int filteringtype, int digitfilteringtype, int digittapetype, string digittypewidth)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_Filtering(channel, coupling, tapewidth, filteringtype, digitfilteringtype, digittapetype, digittypewidth);
        ...
        public override void Oscillograph_Measure_Initialize(int channel)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_Measure_Initialize(channel);
        ...
        public override void Oscillograph_AddMeasure(string MeasurementType, int channel, bool isCAN, int Schannel)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_AddMeasure(MeasurementType, channel, isCAN, Schannel);
        ...
        /// <returns></returns>
        public override Dictionary<int, string> Oscillograph_ReadMeasure(int MeasureNumber)
        {
            Dictionary<int, string> dicMeasure = new Dictionary<int, string>();
            string Measure = "";
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                item.Value.Oscillograph_ReadMeasure(MeasureNumber, ref Measure);
                if (Measure == "")
                {
                    item.Value.Oscillograph_ReadMeasure(MeasureNumber, ref Measure);
                }
                dicMeasure.Add(1, Measure);
        ...
        /// <param name="Schannel">子通道</param>
        /// <returns></returns>
        public override Dictionary<int, string> Oscillograph_ReadMeasure(string MeasureType, int channel, bool isCAN, int Schannel)
        {
            Dictionary<int, string> dicMeasure = new Dictionary<int, string>();
            string Measure = "";
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_ReadMeasure(MeasureType, channel, isCAN, Schannel, ref Measure);
                if (Measure == "")
                {
                    item.Value.Oscillograph_ReadMeasure(MeasureType, channel, isCAN, Schannel, ref Measure);
                }
        ...
        public override void Oscillograph_TimeBase(string timebase)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_TimeBase(timebase);
        ...
        public override void Oscillograph_Trigger(string type_second, int channel, bool isCAN, int schannel, string triggerLevel, string triggerType, double position)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_Trigger(type_second, channel, isCAN, schannel, triggerLevel, triggerType, position);
        ...
        public override void Oscillograph_TriggerTypeSet(string TriggerType)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_TriggerTypeSet(TriggerType);
        ...
        public override Dictionary<int, int> Oscillograph_ReadTrigger()
        {
            Dictionary<int, int> dicMeasure = new Dictionary<int, int>();
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {

                int value = item.Value.Oscillograph_ReadTrigger();
                dicMeasure.Add(1, value);
        ...
        public override Dictionary<int, double[]> OscillographCursorData(int channel, bool isCAN, int schannel)
        {
            Dictionary<int, double[]> dicMeasure = new Dictionary<int, double[]>();

            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {

                double[] value = item.Value.OscillographCursorData(channel, isCAN, schannel);
                if (value == null)
                {
                    value = item.Value.OscillographCursorData(channel, isCAN, schannel);
                }
                dicMeasure.Add(1, value);
        ...
        public override Dictionary<int, string> OscillographSaveScreen()
        {
            Dictionary<int, string> keyValuePairs = new Dictionary<int, string>();
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                string path = "";
                item.Value.OscillographSaveScreen(ref path);
                keyValuePairs.Add(1, path);
            }
        ...
        public override void Oscillograph_IsRun(bool isRun)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_IsRun(isRun);
        ...
        public override Dictionary<int, bool> Oscillograph_ReadRun()
        {
            Dictionary<int, bool> dicMeasure = new Dictionary<int, bool>();

            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {

                bool value = item.Value.Oscillograph_ReadRun();
                dicMeasure.Add(1, value);
        ...
        public override void Oscillograph_CursorsOpen(bool Open)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_CursorsOpen(Open);
        ...
        public override void Oscillograph_MEASureOpen(bool Open)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_MEASureOpen(Open);
        ...
        public override void Oscillograph_CursorsSet(string type)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_CursorsSet(type);
        ...
        public override void Oscillograph_CursorPosition_X(int channel, bool isCAN, int schannel, double value1, double value2)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_CursorPosition_X(channel, isCAN, schannel, value1, value2);
        ...
        public override void Oscillograph_CursorPosition_X_Single(int channel, bool isCAN, int schannel, double value, bool isleft)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_CursorPosition_X_Single(channel, isCAN, schannel, value, isleft);
        ...
        /// <returns></returns>
        public override void Oscillograph_CursorPosition_XY(int channel, bool isCAN, int schannel)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {

                item.Value.Oscillograph_CursorPosition_XY(channel, isCAN, schannel);
        ...
        public override void Oscillograph_CursorPosition_SetChannel(int channel, bool isCAN, int schannel)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_CursorPosition_SetChannel(channel, isCAN, schannel);
        ...
        public override void Oscillograph_CursorPosition_X_Time(int channel, double time, bool isleft)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_CursorPosition_X_Time(channel, time, isleft);
        ...
        public override void Oscillograph_CursorPosition_Y(int trace, double value1, double value2)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_CursorPosition_Y(trace, value1, value2);
        ...
        /// <returns>返回为try catch 的错误，没有则返回为空</returns>
        public override void Oscillograph_CursorPosition_Y_Value(int trace, double value1, double value2)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {

                item.Value.Oscillograph_CursorPosition_Y_Value(trace, value1, value2);
        ...
        public override Dictionary<int, double[]> Oscillograph_ReadCursors(int channel)
        {
            Dictionary<int, double[]> dicMeasure = new Dictionary<int, double[]>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double[] value = item.Value.Oscillograph_ReadCursors(channel);
                dicMeasure.Add(1, value);
        ...
        public override Dictionary<int, double> Oscillograph_ReadCurcor()
        {
            Dictionary<int, double> dicMeasure = new Dictionary<int, double>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double value = item.Value.Oscillograph_ReadCurcor();
                dicMeasure.Add(1, value);
        ...
        public override void Oscillograph_Touch(bool isTouch)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_Touch(isTouch);
        ...
        public override void Oscillograph_StorageDepth(string storagedepth)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_StorageDepth(storagedepth);
        ...
        public override void Oscillograph_SetType(int type)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetType(type);
        ...
        public override void Oscillograph_SetGroup(int group, int format, int GRATicule)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetGroup(group, format, GRATicule);
        ...
        public override void Oscillograph_SetGroup2(int group, int format, int GRATicule, bool FGRid)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetGroup2(group, format, GRATicule, FGRid);
        ...
        public override void Oscillograph_SetGroup3(int group, int format, bool FGRid)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetGroup3(group, format, FGRid);
        ...
        public override void Oscillograph_Group_Show(int group)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_Group_Show(group);
        ...
        public override void Oscillograph_SetGroup_Channel(int trace, int mapping, int group)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetGroup_Channel(trace, mapping, group);
        ...
        public override void Oscillograph_SetCan_Channel(int channel, string BaudRate, bool IsListenOnly, bool Terminator)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetCan_Channel(channel, BaudRate, IsListenOnly, Terminator);
        ...
        public override void Oscillograph_SetCanChild_Open(int channel, int schannel, bool isOpen)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetCanChild_Open(channel, schannel, isOpen);
        ...
        public override void Oscillograph_SetCanChild_Channel(int channel, int schannel, int TRACE, bool isOpen, string SRATE, string Label, int MFORmat, string Mid, string ByteLength, int Start, int ByteCnt, int ByteOrder, int ValueType, string Factor, string Offect, string Unit, string UpLimit, string DownLimit, string Color, bool SGr1, bool SGr2, bool SGr3, bool SGr4)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetCanChild_Channel(channel, schannel, TRACE, isOpen, SRATE, Label, MFORmat, Mid, ByteLength, Start, ByteCnt, ByteOrder, ValueType, Factor, Offect, Unit, UpLimit, DownLimit, Color, SGr1, SGr2, SGr3, SGr4);
        ...
        public override void Oscillograph_SetCanChild_Channel_Renovate(int channel, int schannel, string Mid)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_SetCanChild_Channel_Renovate(channel, schannel, Mid);
        ...
        public override void Remove_All_Channel(int group)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Remove_All_Channel(group);
        ...
        /// <param name="schannel">子通道</param>
        /// <returns>返回值</returns>
        public override Dictionary<int, string> GetChannelValue(int channel, bool isCan, int schannel)
        {
            Dictionary<int, string> dicMeasure = new Dictionary<int, string>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double value = item.Value.GetChannelValue(channel, isCan, schannel);
                dicMeasure.Add(1, value.ToString());
        ...
        public override Dictionary<int, double[]> Oscillograph_Points(double[] data, double upvalue, double downvalue, int uptype, bool isAC, double errorrate)
        {
            Dictionary<int, double[]> dicMeasure = new Dictionary<int, double[]>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double[] value = item.Value.Oscillograph_Points(data, upvalue, downvalue, uptype, isAC, errorrate);
                dicMeasure.Add(1, value);
        ...
        public override Dictionary<int, double> Oscillograph_Points_Single(double[] data, double value, int uptype, bool isright, bool isAC, double errorrate)
        {
            Dictionary<int, double> dicMeasure = new Dictionary<int, double>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double myvalue = item.Value.Oscillograph_Points_Single(data, value, uptype, isright, isAC, errorrate);
                dicMeasure.Add(1, myvalue);

            }
            return dicMeasure;
        }

        public override Dictionary<int, double> Oscillograph_Points_Single(double[] data, double value, int uptype, bool isright, bool isAC, double errorrate, double CheckCount)
        {
            Dictionary<int, double> dicMeasure = new Dictionary<int, double>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double myvalue = item.Value.Oscillograph_Points_Single(data, value, uptype, isright, isAC, errorrate, CheckCount);
                dicMeasure.Add(1, myvalue);
        ...
        /// <param name="errorrate">误差</param>
        /// <returns>返回double，返回占屏幕的比例值，跟录波仪没有关系，自己根据录波仪回读的点来进行计算</returns>
        public override Dictionary<int, double> Oscillograph_Points_Single2(double[] data, double value, int uptype, bool isright, bool isAC, double errorrate, int? CheckCount)
        {
            Dictionary<int, double> dicMeasure = new Dictionary<int, double>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double myvalue = item.Value.Oscillograph_Points_Single2(data, value, uptype, isright, isAC, errorrate, CheckCount);
                dicMeasure.Add(1, myvalue);
        ...
        public override Dictionary<int, double> Oscillograph_Points_Single3(double[] data, double value, bool isAC)
        {
            Dictionary<int, double> dicMeasure = new Dictionary<int, double>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double myvalue = item.Value.Oscillograph_Points_Single3(data, value, isAC);
                dicMeasure.Add(1, myvalue);
        ...
        public override Dictionary<int, double> Oscillograph_Points_Single_Multiple(double[] OldData, double value, int uptype, bool isright, bool isAC, double errorrate, int Index, int Count, int? CheckCount = null)
        {
            Dictionary<int, double> dicMeasure = new Dictionary<int, double>();
            ...
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                double myvalue = item.Value.Oscillograph_Points_Single_Multiple(OldData, value, uptype, isright, isAC, errorrate, ref Index, Count, CheckCount);
                dicMeasure.Add(1, myvalue);
        ...
        public override void Oscillograph_STRain_LSCale(int channel, string mode, double AVALue, double BVALue, string Unit, string DISPlaytype)
        {
            foreach (KeyValuePair<int, EquipMentBase> item in DitControlEquipMent(ClassNames))
            {
                ...
                item.Value.Oscillograph_STRain_LSCale(channel, mode, AVALue, BVALue, Unit, DISPlaytype);
